                   ...
                   double varargin_3, double varargin_4, double varargin_5)
{
  ...
  /*      end */
  /*  end */
  /* 'getgkmweights:18' fn = varargin{1}; */
  /* 'getgkmweights:19' l_svm = varargin{2}; */
  /* 'getgkmweights:20' k_svm = varargin{3}; */
  /* 'getgkmweights:21' RC = varargin{4}; */
  /* 'getgkmweights:22' nfrac = varargin{5}; */
  /* 'getgkmweights:23' lk = 1; */
  lk_size[0] = 1;
  lk_size[1] = 1;
  lk_data[0] = 1.0;
  /*  if nargin > 3 */
  ...
  /*      end */
  /*  end */
  /* 'getgkmweights:35' if nfrac ~= 1 */
  if (varargin_5 != 1.0) {
    /* 'getgkmweights:36' lk = [l_svm k_svm]; */
    lk_size[0] = 1;
    lk_size[1] = 2;
  ...
  /* 'getgkmweights:39' [comb,comb2,diffc,indc,xc,rcnum] =
   * genIndex(l_svm,k_svm,nfrac); */
  genIndex(varargin_2, varargin_3, varargin_5, comb, c2, negvec, cfile, seqvec2,
           &rcnum);
  comb_data = comb->data;
  /* 'getgkmweights:40' if numel(comb)/k_svm*4^k_svm > 6*10^5 */
  c = pow(4.0, varargin_3);
  if ((double)(comb->size[0] * comb->size[1]) / varargin_3 * c > 600000.0) {
    /* 'getgkmweights:41' nfrac = round(5*10^7/4^k_svm/numel(comb)*k_svm)/100;
     */
    nfrac = rt_roundd(5.0E+7 / c / (double)(comb->size[0] * comb->size[1]) *
                      varargin_3) /
            100.0;
    /* 'getgkmweights:42' fprintf('Combination of (l,k) yields too many gapped
    ...
    printf("Combination of (l,k) yields too many gapped kmers.  Using %f of "
           ...
           nfrac);
    fflush(stdout);
    /* 'getgkmweights:43' lk = ([l_svm k_svm]); */
    lk_size[0] = 1;
    lk_size[1] = 2;
    lk_data[0] = varargin_2;
    lk_data[1] = varargin_3;
    /* 'getgkmweights:44' [comb,comb2,diffc,indc,xc,rcnum] =
     * genIndex(l_svm,k_svm,nfrac); */
    genIndex(varargin_2, varargin_3, nfrac, comb, c2, negvec, cfile, seqvec2,
             &rcnum);
    comb_data = comb->data;
  }
  /* 'getgkmweights:46' fprintf('Counting gapped kmers\n'); */
  printf("Counting gapped kmers\n");
  fflush(stdout);
  /* 'getgkmweights:47' [cfile, GCpos1, GCneg1,mat,mat2] = getgkmcounts(fn,
   * l_svm, k_svm, lk, RC, comb, rcnum); */
  getgkmcounts(varargin_1, varargin_2, varargin_3, lk_data, lk_size, varargin_4,
               comb, rcnum, cfile, &b_varargin_1, &GCneg1, mat, mat2);
  cfile_data = cfile->data;
  /* 'getgkmweights:48' negvec = BGkmer(mat, GCneg1,comb,rcnum,l_svm,k_svm,RC);
   */
  /* 'BGkmer:2' len = length(c); */
  if ((comb->size[0] == 0) || (comb->size[1] == 0)) {
    u1 = 0;
  } else {
    u1 = comb->size[0] * comb->size[1] / varargin_3;
  }
  ...
  for (b_i = 0; b_i < i1; b_i++) {
    ...
    for (i2 = 0; i2 < 4; i2++) {
      for (i3 = 0; i3 < 4; i3++) {
        j2 = i3 << 2;
        mat2[i2 + j2] = ((p_data[b_i].f1[i2] * mat[j2] +
                          p_data[b_i].f1[i2 + 4] * mat[j2 + 1]) +
                         p_data[b_i].f1[i2 + 8] * mat[j2 + 2]) +
                        p_data[b_i].f1[i2 + 12] * mat[j2 + 3];
      }
    }
    for (i2 = 0; i2 < 16; i2++) {
  ...
  for (b_i = 0; b_i < u1; b_i++) {
    ...
    for (i1 = 0; i1 < 4; i1++) {
      j2 = i1 << 2;
      startvec[i1] = ((nfrac * mat2[j2] + GCmat_idx_1 * mat2[j2 + 1]) +
                      GCmat_idx_2 * mat2[j2 + 2]) +
                     nfrac * mat2[j2 + 3];
    }
    /* 'BGkmer:30' if RC */
    if (varargin_4 != 0.0) {
    ...
    for (nd2 = 0; nd2 < i1; nd2++) {
      ...
      j2 = (int)(varargin_3 - 1.0);
      for (b_j1 = 0; b_j1 < j2; b_j1++) {
        /* 'BGkmer:40' matt = p{dc(iii)+1}; */
        /* 'BGkmer:41' negweights(a) = negweights(a)*matt(seqvec(ii,iii),
         * seqvec(ii,iii+1)); */
        negvec_data[m] *=
            p_data[(int)(dc_data[b_j1] + 1.0) - 1]
                .f1[((int)seqvec_data[nd2 + seqvec->size[0] * b_j1] +
                     (((int)seqvec_data[nd2 + seqvec->size[0] * (b_j1 + 1)] - 1)
                      << 2)) -
                    1];
        /* 'BGkmer:42' if RC */
        if (varargin_4 != 0.0) {
          ...
          tmp *=
              p_data[(int)(dc2_data[b_j1] + 1.0) - 1].f1
                  [((int)seqvec2_data[nd2 + seqvec2->size[0] * b_j1] +
                    (((int)seqvec2_data[nd2 + seqvec2->size[0] * (b_j1 + 1)] -
                      1)
                     << 2)) -
                   1];
        }
  ...
  if ((rcnum > 0.0) && (varargin_4 != 0.0)) {
    ...
    nd2 = (m - j2) - 1;
    m = b_comb->size[0] * b_comb->size[1];
    b_comb->size[0] = 1;
    b_comb->size[1] = nd2;
    emxEnsureCapacity_real_T(b_comb, m);
    b_comb_data = b_comb->data;
    for (m = 0; m < nd2; m++) {
      b_comb_data[m] = negvec_data[(i1 + m) - 1] / 1.4142135623730951;
    }
    nd2 = b_comb->size[1];
    for (i1 = 0; i1 < nd2; i1++) {
      negvec_data[(j2 + i1) + 1] = b_comb_data[i1];
    }
  }
  emxFree_real_T(&b_comb);
  /* 'getgkmweights:49' cfile = cfile-negvec/sum(negvec)*sum(cfile); */
  nfrac = blockedSummation(cfile, cfile->size[0]);
  GCmat_idx_1 = blockedSummation(negvec, negvec->size[0]);
  if (cfile->size[0] == negvec->size[0]) {
    nd2 = cfile->size[0];
    for (i1 = 0; i1 < nd2; i1++) {
      cfile_data[i1] -= negvec_data[i1] / GCmat_idx_1 * nfrac;
    }
  } else {
    binary_expand_op(cfile, negvec, GCmat_idx_1, nfrac);
    cfile_data = cfile->data;
  }
  emxFree_real_T(&negvec);
  /* 'getgkmweights:50' cfile = cfile/std(cfile); */
  nfrac = b_std(cfile);
  nd2 = cfile->size[0];
  for (i1 = 0; i1 < nd2; i1++) {
    cfile_data[i1] /= nfrac;
  ...
  s->size[0] = 1;
  s->size[1] = 0;
  /* 'getgkmweights:52' for i = 1:k_svm */
  for (b_i = 0; b_i < i3; b_i++) {
    /* 'getgkmweights:53' s = [s 'A']; */
    i1 = s->size[1];
    j2 = s->size[0] * s->size[1];
    s->size[1]++;
    emxEnsureCapacity_char_T(s, j2);
    s_data = s->data;
    s_data[i1] = 'A';
  }
  emxInit_char_T(&kmer, 2);
  /* 'getgkmweights:55' kmer = repmat(s,4^k_svm,1); */
  ...
  /* 'getgkmweights:56' vec = 0:4^k_svm-1; */
  if (c - 1.0 < 0.0) {
    dc->size[0] = 1;
    dc->size[1] = 0;
  } else {
  ...
  /* 'getgkmweights:57' for i = 1:k_svm */
  emxInit_int32_T(&r, 1);
  emxInit_int32_T(&r1, 2);
  emxInit_boolean_T(&b_dc2, 2);
  for (b_i = 0; b_i < i3; b_i++) {
    /* 'getgkmweights:58' vec2 = mod(floor(vec/4^(k_svm-i)),4); */
    c = pow(4.0, varargin_3 - ((double)b_i + 1.0));
    ...
    /* 'getgkmweights:59' f = find(vec2==1); */
    /* 'getgkmweights:60' kmer(f,i) = 'C'; */
    ...
    /* 'getgkmweights:61' f = find(vec2==2); */
    /* 'getgkmweights:62' kmer(f,i) = 'G'; */
    i1 = b_dc2->size[0] * b_dc2->size[1];
    b_dc2->size[0] = 1;
    b_dc2->size[1] = dc2->size[1];
    emxEnsureCapacity_boolean_T(b_dc2, i1);
    ...
    /* 'getgkmweights:63' f = find(vec2==3); */
    /* 'getgkmweights:64' kmer(f,i) = 'T'; */
    i1 = b_dc2->size[0] * b_dc2->size[1];
    b_dc2->size[0] = 1;
    b_dc2->size[1] = dc2->size[1];
    emxEnsureCapacity_boolean_T(b_dc2, i1);
  ...
  s->size[0] = 1;
  s->size[1] = 0;
  /* 'getgkmweights:67' for i = 1:l_svm */
  for (b_i = 0; b_i < i; b_i++) {
    /* 'getgkmweights:68' s = [s '-']; */
    i1 = s->size[1];
  ...
  /* 'getgkmweights:70' a = 1; */
  a = 1U;
  /* 'getgkmweights:71' fid = fopen(sprintf('%s_%d_%d_gkmweights.out', fn,
  ...
  b_varargin_1_data = c_varargin_1->data;
  ...
  /* 'getgkmweights:72' for i = 1:numel(comb)/k_svm */
  ...
  for (b_i = 0; b_i < i; b_i++) {
    /* 'getgkmweights:73' for j = 1:4^k_svm */
    for (m = 0; m < i4; m++) {
      /* 'getgkmweights:74' S = s; */
      ...
      /* 'getgkmweights:75' S(comb(i,:)) = kmer(j,:); */
      for (i1 = 0; i1 < b_loop_ub; i1++) {
        S_data[(int)comb_data[b_i + comb->size[0] * i1] - 1] =
            kmer_data[m + kmer->size[0] * i1];
      }
      /* 'getgkmweights:76' fprintf(fid, '%s\t%0.5f\n', S, cfile(a)); */
      i1 = c_varargin_1->size[0] * c_varargin_1->size[1];
      c_varargin_1->size[0] = 1;
      c_varargin_1->size[1] = S->size[1] + 1;
      emxEnsureCapacity_char_T(c_varargin_1, i1);
      b_varargin_1_data = c_varargin_1->data;
      nd2 = S->size[1];
      for (i1 = 0; i1 < nd2; i1++) {
  ...
  emxFree_real_T(&comb);
  /* 'getgkmweights:80' fclose(fid); */
  cfclose(fileid);
  /*  dlmwrite([fn '_negmat.out'], [mat; GCpos1 GCneg1 0 0], 'precision',10); */
  /* 'getgkmweights:82' fid = fopen(sprintf('%s_negmat.out', fn), 'w'); */
  i = c_varargin_1->size[0] * c_varargin_1->size[1];
  c_varargin_1->size[0] = 1;
  c_varargin_1->size[1] = varargin_1->size[1] + 1;
  emxEnsureCapacity_char_T(c_varargin_1, i);
  ...
  emxInit_char_T(&b_filename, 2);
  S_data[varargin_1->size[1]] = '\x00';
  nd2 = snprintf(NULL, 0, "%s_negmat.out", &S_data[0]);
  i = b_filename->size[0] * b_filename->size[1];
  b_filename->size[0] = 1;
  b_filename->size[1] = nd2 + 1;
  emxEnsureCapacity_char_T(b_filename, i);
  S_data = b_filename->data;
  ...
  if (1 > nd2) {
    ...
    b_filename->size[1] = nd2;
  }
  emxEnsureCapacity_char_T(b_filename, i);
  fileid = cfopen(b_filename, "wb");
  /* 'getgkmweights:83' matsize = size(mat); */
  /* 'getgkmweights:84' for i=1:matsize(1) */
  b_NULL = NULL;
  /* 'getgkmweights:85' fprintf(fid, '%.10f,%.10f,%.10f,%.10f\n', mat(i,1),
   * mat(i,2), mat(i,3), mat(i,4)); */
  getfilestar(fileid, &filestar, &autoflush);
  emxFree_char_T(&c_varargin_1);
  emxFree_char_T(&b_filename);
  emxFree_char_T(&S);
  if (!(filestar == b_NULL)) {
  ...
  /* 'getgkmweights:87' fprintf(fid, '%.10f,%.10f,%.10f,%.10f\n', GCpos1,
   * GCneg1, 0.0, 0.0); */
  b_NULL = NULL;
  getfilestar(fileid, &filestar, &autoflush);
  if (!(filestar == b_NULL)) {
    fprintf(filestar, "%.10f,%.10f,%.10f,%.10f\n", b_varargin_1, GCneg1, 0.0,
            0.0);
    if (autoflush) {
      fflush(filestar);
    }
