                  ...
                  emxArray_cell_wrap_2 *names, double a, double rnum)
{
  ...
  /* 'mapTF:153' GCmat = [0.5-GC/2 GC/2 GC/2 0.5-GC/2]; */
  /* 'mapTF:154' L = length(ss)-l_svm+1; */
  L = ((double)ss->size[1] - l_svm) + 1.0;

  /* 'mapTF:155' ACGT = 'ACGT'; */
  /* 'mapTF:156' lab = [lab; 0]; */
  /* 'mapTF:157' n = sum(LEN)+1; */
  y = blockedSummation(LEN, LEN->size[0]);

  /* 'mapTF:158' mat = zeros(n,L)-Inf; */
  ...
  /* 'mapTF:164' gkmprob(gkmprob>0.99) = 0.99; */
  nx = gkmprob->size[0];
  ...
  /* 'mapTF:165' pos = gkmprob; */
  /* 'mapTF:166' neg = log(1-gkmprob); */
  i = neg->size[0];
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:172' TRANS(C(i+1),n)=1; */
    TRANS_data[((int)C_data[b_i + 1] + TRANS->size[0] * ((int)(y + 1.0) - 1)) -
      1] = 1.0;
  }

  /* 'mapTF:174' TRANS(n,C(1:end-1)+1) = 1; */
  if (1 > C->size[0] - 1) {
    b_loop_ub = 0;
  } else {
    b_loop_ub = C->size[0] - 1;
  ...
  /* 'mapTF:177' for i = 1:a */
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:178' mat(C(i)+1,1) = pwm_prob(C(i)+1,1)+pos(1); */
    nx = (int)(C_data[b_i] + 1.0) - 1;
    mat_data[nx] = pwm_prob_data[nx] + gkmprob_data[0];
  }

  /* 'mapTF:180' mat(n,1) = neg(1); */
  ...
  for (b_i = 0; b_i < i1; b_i++) {
    /* 'mapTF:182' for j = 1:a */
    for (k = 0; k < i; k++) {
      /* 'mapTF:183' [mat(C(j)+1,i),ind(C(j)+1,i)] = max(mat(1:end,i-1)+TRANS(1:end,C(j)+1)+pwm_prob(C(j)+1,i)); */
      if (1 > mat->size[0]) {
        b_loop_ub = 0;
      } else {
      ...
      /* 'mapTF:184' for jj = 1:LEN(j+1)-1 */
      i2 = (int)(LEN_data[k + 1] - 1.0);
      for (nx = 0; nx < i2; nx++) {
        /* 'mapTF:185' mat(C(j)+jj+1,i) = mat(C(j)+jj,i-1)+TRANS(C(j)+jj,C(j)+jj+1)+pwm_prob(C(j)+jj+1,i); */
        ...
        /* 'mapTF:186' ind(C(j)+jj+1,i) = C(j)+jj; */
        ind_data[((int)(TRANS_tmp + 1.0) + ind->size[0] * (b_i + 1)) - 1] =
          TRANS_tmp;
      }
    }

    /* 'mapTF:189' [mat(n,i),ind(n,i)] = max(mat(1:end,i-1)+TRANS(1:end,end)+neg(i-1)); */
    if (1 > mat->size[0]) {
      b_loop_ub = 0;
    } else {
      b_loop_ub = mat->size[0];
  ...
  /* 'mapTF:192' path(end) = n; */
  path_data[(int)L - 1] = y + 1.0;

  /*  for i = fliplr(1:L-1) */
  /* 'mapTF:194' for i = L-1:-1:1 */
  i1 = (int)-((-1.0 - (L - 1.0)) + 1.0);
  for (b_i = 0; b_i < i1; b_i++) {
    TRANS_tmp = (L - 1.0) + -(double)b_i;

    /* 'mapTF:195' path(i) = ind(path(i+1),i+1); */
  ...
  /* 'mapTF:197' PATH = zeros(length(s),1); */
  i1 = PATH->size[0];
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:201' if ~isempty(f) */
    if (f->size[0] != 0) {
      /* 'mapTF:202' for j = 1:length(f) */
      i1 = f->size[0];
      for (k = 0; k < i1; k++) {
        /* 'mapTF:203' vec = f(j)+shift(i):f(j)+shift(i)+LEN_2(i)-1; */
        ...
        /* 'mapTF:204' PATH(vec) = C_2(i)+1:C_2(i)+LEN_2(i); */
  ...
  /* 'mapTF:208' N =sum(LEN_2)+1; */
  y = blockedSummation(LEN_2, LEN_2->size[0]);

  /* 'mapTF:209' PATH(PATH==0) = N; */
  nx = PATH->size[0];
  for (b_i = 0; b_i < nx; b_i++) {
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:214' lab = [lab;ones(LEN_2(i),1)*i]; */
    i1 = neg->size[0];
  ...
  /* 'mapTF:218' n = sum(LEN_2)+1; */
  y = blockedSummation(LEN_2, LEN_2->size[0]);

  /* 'mapTF:219' PWM = ones(L,4)/4; */
  /* 'mapTF:220' PWM_alt = ones(L,4)/4; */
  /* 'mapTF:222' GCmat = [0.5-GC/2 GC/2 GC/2 0.5-GC/2]; */
  /* 'mapTF:224' I = lab(PATH); */
  /* 'mapTF:225' Cmat = [I PATH ones(length(I),4)/4]; */
  if (PATH->size[0] != 0) {
    result = PATH->size[0];
  } else {
    result = 0;
  ...
  /* 'mapTF:226' for i = 1:L+l_svm-1 */
  i = (int)((L + l_svm) - 1.0);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:227' if PATH(i)~=n */
    if (PATH_data[b_i] != y + 1.0) {
      /* 'mapTF:228' Cmat(i,3:6) = MAT(PATH(i),:); */
      ...
      /* 'mapTF:229' else */
      /* 'mapTF:230' Cmat(i,2) = 0; */
      TRANS_data[b_i + Cmat->size[0]] = 0.0;
    }
  }

  /* 'mapTF:233' Cmat(:,3:6) = (Cmat(:,3:6)+0.0001)/(1.0001); */
  nx = Cmat->size[0] - 1;
  i = b_MAT->size[0] * b_MAT->size[1];
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:240' if ~isempty(f) */
    if (f->size[0] != 0) {
      /* 'mapTF:241' for j = 1:length(f) */
      i1 = f->size[0];
      for (k = 0; k < i1; k++) {
        /* 'mapTF:242' if f(j)+shift(i)+LEN_2(i)< length(ss)-l_svm+1 && f(j)+shift(i) > l_svm-1 */
        TRANS_tmp = (double)f_data[k] + shift_data[b_i];
        if ((TRANS_tmp + LEN_2_data[b_i] < ((double)ss->size[1] - l_svm) + 1.0) &&
          ...
          /* 'mapTF:244' F = [F; f(j)+shift(i) f(j)+shift(i)+LEN_2(i)-1 mean(gkmprob(f(j):f(j)+2*shift(i)+LEN_2(i)-l_svm))]; */
          i2 = f_data[k];
  ...
  /* 'mapTF:252' NAME = cell(numel(L)/4,1); */
  ...
  /* 'mapTF:253' NAME = coder.nullcopy(NAME); */
  /* 'mapTF:254' Lmat = zeros(numel(L)/4,4); */
  i = MAT->size[0] * MAT->size[1];
  ...
  /* 'mapTF:255' if ~isempty(L) */
  if ((b_L->size[0] != 0) && (b_L->size[1] != 0)) {
    /* 'mapTF:256' [~,b] = sort(L(:,1)); */
    loop_ub = b_L->size[0];
    i = neg->size[0];
    ...
    /* 'mapTF:258' for i = 1:numel(L)/4 */
    i = (int)((double)(b_L->size[0] * b_L->size[1]) / 4.0);
    for (b_i = 0; b_i < i; b_i++) {
      /* 'mapTF:259' NAME{i} = names{L(i,4)}; */
      ...
      /* 'mapTF:260' Lmat(i,:) = [L(i,4) L(i,1) L(i,2) L(i,3)]; */
  ...
  /* 'mapTF:263' kmer = scoreseqkmer(fn, rnum, ss, Smat, l_svm,k_svm, ofn,Cmat, dsvm); */
  scoreseqkmer(ss, Smat, l_svm, Cmat, dsvm, F);

  /* 'mapTF:264' [omat, NAME, Lmat] = PWM_corr(ofn, rnum, mfn,kmer,Cmat, dsvm, NAME, Lmat,PWM2); */
  i = PATH->size[0];
  ...
  /* 'mapTF:265' kmer = scoreseqkmer(fn, rnum, ss, Smat, l_svm, k_svm,ofn,omat, dsvm); */
  scoreseqkmer(ss, Smat, l_svm, Cmat, dsvm, F);

  /* 'mapTF:266' PWM_corr2(ofn, rnum, kmer, dsvm, NAME, Lmat, s) */
  i = PATH->size[0];
  PATH->size[0] = dsvm->size[0];
  emxEnsureCapacity_real_T(PATH, i);
  PATH_data = PATH->data;
                     ...
                     emxArray_real_T *Lmat2)
{
  ...
  /* 'mapTF:375' [coords, ind] = sort(kmer(:,1)); */
  loop_ub = kmer->size[0];
  i = LEN->size[0];
  ...
  /* 'mapTF:378' L = length(NAME); */
  /* 'mapTF:379' ind = ones(L,1); */
  /* 'mapTF:380' n = []; */
  n->size[0] = 0;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:382' F = find(coords > Lmat(i,2)); */
    /* 'mapTF:383' FF = find(coords <= Lmat(i,3)); */
    /* 'mapTF:384' f = intersect(F,FF); */
    ...
    /* 'mapTF:385' a1 = ip(kmer(f,end),dsvm(f,end)); */
    loop_ub = f->size[0];
    i1 = b_dsvm->size[0];
    ...
    /* 'mapTF:426' c = x'*y/sqrt(x'*x)/sqrt(y'*y); */
    if (f->size[0] < 1) {
      b_Lmat = 0.0;
      x = 0.0;
      b_x = 0.0;
    ...
    if (b_Lmat / x / b_x > 0.6) {
      ...
      /* 'mapTF:388' else */
      /* 'mapTF:389' ind(i) = 0; */
      /* 'mapTF:390' omat(Lmat(i,2):Lmat(i,3),1:2) = 0; */
      b_Lmat = Lmat_data[b_i + Lmat->size[0]];
      ...
      /* 'mapTF:391' omat(Lmat(i,2):Lmat(i,3),3:6) = 0.25; */
      b_Lmat = Lmat_data[b_i + Lmat->size[0]];
      x = Lmat_data[b_i + Lmat->size[0] * 2];
      if (b_Lmat > x) {
        i1 = 0;
  ...
  /*  NAME2 = NAME(n); */
  /* 'mapTF:396' NAME2_new_length = length(n); */
  /* 'mapTF:397' NAME2_new = cell(NAME2_new_length,1); */
  /* 'mapTF:398' for idx=1:NAME2_new_length */
  i = n->size[0];
  i1 = NAME2->size[0];
  ...
  for (dsvm_tmp = 0; dsvm_tmp < i; dsvm_tmp++) {
    /* 'mapTF:399' NAME2_new{idx} = NAME{n(idx)}; */
  ...
  /* 'mapTF:401' NAME2 = NAME2_new; */
  /* 'mapTF:402' LEN = zeros(length(p),1); */
  /* 'mapTF:403' for i = 1:length(p) */
  /* 'mapTF:406' C = cumsum([0;LEN]); */
  /* 'mapTF:407' mat = [zeros(length(omat),2) ones(length(omat),4)/4]; */
  /* 'mapTF:408' for i = 1:L-length(n) */
}
                      ...
                      s)
{
  ...
  /* 'mapTF:511' [coords, ind] = sort(kmer(:,1)); */
  loop_ub = kmer->size[0];
  i = coords->size[0];
  ...
  /* 'mapTF:514' L = length(NAME); */
  /* 'mapTF:515' if NUM ==1 */
  emxInit_char_T(&b_fn, 2);
  if (NUM == 1.0) {
    /* 'mapTF:516' fid1 = fopen([fn '_kmer_PWM_locs.out'],'w'); */
    ...
    /* 'mapTF:517' else */
    /* 'mapTF:518' fid1 = fopen([fn '_kmer_PWM_locs.out'],'a+'); */
    i = b_fn->size[0] * b_fn->size[1];
    b_fn->size[0] = 1;
    b_fn->size[1] = fn->size[1] + 18;
    emxEnsureCapacity_char_T(b_fn, i);
  ...
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:522' F = find(coords > Lmat(i,2)); */
    /* 'mapTF:523' FF = find(coords <= Lmat(i,3)); */
    /* 'mapTF:524' f = intersect(F,FF); */
    b_Lmat = Lmat_data[b_i + Lmat->size[0]];
    i1 = b_coords->size[0];
    ...
    /* 'mapTF:525' a1 = ip(kmer(f,end),dsvm(f,end)); */
    loop_ub = f->size[0];
    i1 = b_dsvm->size[0];
    b_dsvm->size[0] = f->size[0];
    emxEnsureCapacity_real_T(b_dsvm, i1);
    ...
    /* 'mapTF:526' fprintf(fid1, '%d\t%s\t%d\t%d\t%d\t%f\t%f\t%s\n', int32(NUM), NAME{i}, ... */
    /* 'mapTF:527'         int32(Lmat(i,1)), int32(Lmat(i,2)), int32(Lmat(i,3)), Lmat(i,4), a1, s(Lmat(i,2):Lmat(i,3))); */
                     ...
                     emxArray_cell_wrap_2 *names, emxArray_real_T *len)
{
  ...
  do {
    ...
    if (i == 0) {
      /* 'mapTF:436' line = fgetl(fid); */
      fgetl(fid, line);
      line_data = line->data;

      /* 'mapTF:437' if length(line) >= 5 && strcmp(line(1:5), 'MOTIF') */
      if (line->size[1] >= 5) {
        for (i = 0; i < 5; i++) {
          a[i] = line_data[i];
        }
  ...
  /* 'mapTF:441' fseek(fid, curr_pos, 'bof'); */
  wherefrom = SEEK_SET;
  ...
  /* 'mapTF:444' mat = coder.nullcopy(mat); */
  /* 'mapTF:445' names = cell(idx, 1); */
  i = names->size[0];
  ...
  /* 'mapTF:446' names = coder.nullcopy(names); */
  /* 'mapTF:447' len = zeros(idx, 1); */
  i = len->size[0];
  len->size[0] = (int)idx;
  emxEnsureCapacity_real_T(len, i);
  len_data = len->data;
  ...
  do {
    ...
    if (i == 0) {
      /* 'mapTF:451' line = fgetl(fid); */
      fgetl(fid, line);
      line_data = line->data;

      /* 'mapTF:452' if length(line) < 5 || ~strcmp(line(1:5), 'MOTIF') */
      if (line->size[1] >= 5) {
        for (i = 0; i < 5; i++) {
          a[i] = line_data[i];
        }
        ...
        if (ret == 0) {
          ...
          /* 'mapTF:456' names{i} = fgetl(fid); */
          fgetl(fid, names_data[(int)b_i - 1].f1);

          /* 'mapTF:457' len(i) = real(str2double(fgetl(fid))); */
          fgetl(fid, line);
          dc = str2double(line);
          len_data[(int)b_i - 1] = dc.re;

          /* 'mapTF:458' tmp = zeros(len(i), 4); */
          ...
          /* 'mapTF:459' line_count = 1; */
          line_count = 1U;

          /* 'mapTF:460' line = fgetl(fid); */
          fgetl(fid, line);

          /* 'mapTF:461' while ~isempty(line) */
          while (line->size[1] != 0) {
            /* 'mapTF:462' [v1, remain] = strtok(line); */
            b_strtok(line, v1, remain);

            /* 'mapTF:463' [v2, remain] = strtok(remain); */
            b_strtok(remain, v2, b_remain);

            /* 'mapTF:464' [v3, v4] = strtok(remain); */
            b_strtok(b_remain, line, remain);

            /* 'mapTF:465' vals = [real(str2double(v1)),  */
            /* 'mapTF:466'                 real(str2double(v2)),  */
            /* 'mapTF:467'                 real(str2double(v3)),  */
            /* 'mapTF:468'                 real(str2double(v4))]'; */
            dc = str2double(v1);
            dc1 = str2double(v2);
            dc2 = str2double(line);
            ...
            /* 'mapTF:469' tmp(line_count,:) = vals; */
            /* 'mapTF:470' line = fgetl(fid); */
            fgetl(fid, line);

            /* 'mapTF:471' line_count = line_count + 1; */
            line_count++;
          }
  ...
  *mat, emxArray_real_T *w)
{
  ...
  do {
    ...
    if (i == 0) {
      /* 'mapTF:971' line = fgetl(fid); */
      fgetl(fid, line);
      line_data = line->data;

      /* 'mapTF:972' if length(line) >= 5 && strcmp(line(1:5), 'MOTIF') */
      if (line->size[1] >= 5) {
        for (i = 0; i < 5; i++) {
          a[i] = line_data[i];
        }
  ...
  /* 'mapTF:976' fseek(fid, curr_pos, 'bof'); */
  wherefrom = SEEK_SET;
  if ((!rtIsInf(curr_pos)) && (!rtIsNaN(curr_pos)) && (floor(curr_pos) ==
       curr_pos)) {
    getfilestar(fid, &filestar, &b_bool);
  ...
  /* 'mapTF:979' mat = coder.nullcopy(mat); */
  /* 'mapTF:980' w = zeros(idx, 1); */
  i = w->size[0];
  w->size[0] = (int)idx;
  ...
  do {
    ...
    if (i == 0) {
      ...
      /* 'mapTF:985' if length(line) < 5 || ~strcmp(line(1:5), 'MOTIF') */
      if (line->size[1] >= 5) {
        for (i = 0; i < 5; i++) {
          a[i] = line_data[i];
        }
        ...
        if (ret == 0) {
          ...
          /* 'mapTF:989' line = fgetl(fid); */
          fgetl(fid, line);
          line_data = line->data;

          /* 'mapTF:991' [~, remain] = strtok(line); */
          b_strtok(line, a__10, remain);

          /* 'mapTF:992' [curr_w, remain] = strtok(remain); */
          b_strtok(remain, curr_w, b_remain);

          /* 'mapTF:993' [~, remain] = strtok(remain); */
          b_strtok(b_remain, a__10, remain);

          /* 'mapTF:994' [curr_alphabet, remain] = strtok(remain); */
          b_strtok(remain, curr_alphabet, b_remain);

          /* 'mapTF:995' [~, remain] = strtok(remain); */
          b_strtok(b_remain, a__10, remain);

          /* 'mapTF:996' [curr_length, ~] = strtok(remain); */
          b_strtok(remain, a__10, a__13);

          /* 'mapTF:997' w(i) = real(str2double(curr_w)); */
          dc = str2double(curr_w);
          w_data[(int)b_i - 1] = dc.re;

          /* 'mapTF:998' curr_alphabet = real(str2double(curr_alphabet)); */
          dc = str2double(curr_alphabet);

          /* 'mapTF:999' curr_length = real(str2double(curr_length)); */
          dc1 = str2double(a__10);

          /* 'mapTF:1000' tmp = zeros(curr_length, curr_alphabet); */
          ...
          /* 'mapTF:1001' line_count = 1; */
          line_count = 1U;

          /* 'mapTF:1002' while ~isempty(line) */
          while (line->size[1] != 0) {
            /* 'mapTF:1003' if strfind(line, 'weight') */
            ...
            if (match_out->size[1] != 0) {
              /* 'mapTF:1004' line = fgetl(fid); */
              fgetl(fid, line);
            }

            /* 'mapTF:1006' [v1, remain] = strtok(line); */
            b_strtok(line, curr_w, remain);

            /* 'mapTF:1007' [v2, remain] = strtok(remain); */
            b_strtok(remain, curr_alphabet, b_remain);

            /* 'mapTF:1008' [v3, v4] = strtok(remain); */
            b_strtok(b_remain, a__10, a__13);

            /* 'mapTF:1009' vals = [real(str2double(v1)),  */
            /* 'mapTF:1010'                 real(str2double(v2)),  */
            /* 'mapTF:1011'                 real(str2double(v3)),  */
            /* 'mapTF:1012'                 real(str2double(v4))]'; */
            dc = str2double(curr_w);
            ...
            /* 'mapTF:1013' tmp(line_count,:) = vals; */
            ret = tmp->size[1];
            for (i = 0; i < ret; i++) {
              tmp_data[((int)line_count + tmp->size[0] * i) - 1] = vals[i];
            }

            /* 'mapTF:1014' line = fgetl(fid); */
            fgetl(fid, line);
            line_data = line->data;

            /* 'mapTF:1015' line_count = line_count + 1; */
            line_count++;
          }

          /* 'mapTF:1017' mat{i} = tmp; */
          i = mat_data[(int)b_i - 1].f1->size[0] * mat_data[(int)b_i - 1]
            .f1->size[1];
          mat_data[(int)b_i - 1].f1->size[0] = tmp->size[0];
...
static void letterconvert(const emxArray_char_T *s, emxArray_real_T *en)
{
  ...
  /* 'mapTF:360' l = length(s); */
  /* 'mapTF:361' en = zeros(1,l); */
  ...
  i = s->size[1];
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:363' if strcmp(s(i),'A') || strcmp(s(i), 'a') */
    c = s_data[b_i];
    if ((!(c != 'A')) || (!(c != 'a'))) {
      /* 'mapTF:364' en(i) = 0; */
      en_data[b_i] = 0.0;
    } else if ((!(c != 'C')) || (!(c != 'c'))) {
      /* 'mapTF:365' elseif strcmp(s(i),'C') || strcmp(s(i),'c') */
      /* 'mapTF:366' en(i) = 1; */
      en_data[b_i] = 1.0;
    } else if ((!(c != 'G')) || (!(c != 'g'))) {
      /* 'mapTF:367' elseif strcmp(s(i),'G') || strcmp(s(i),'g') */
      /* 'mapTF:368' en(i) = 2; */
      en_data[b_i] = 2.0;
    } else {
      /* 'mapTF:369' else */
                   ...
                   double *ind, double *M)
{
  ...
  /* 'mapTF:936' n = length(lenvec)-1; */
  /* 'mapTF:937' simmat = ones(n-1,1); */
  /* 'mapTF:938' for i = 1:n+1 */
  i = lenvec->size[0];
  emxInit_real_T(&diag_a, 2);
  emxInit_real_T(&A, 2);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:939' mot{i} = mot{i}-1/4; */
    loop_ub = mot_data[b_i].f1->size[0] * mot_data[b_i].f1->size[1];
    for (i1 = 0; i1 < loop_ub; i1++) {
      mot_data[b_i].f1->data[i1] -= 0.25;
    }

    /* 'mapTF:940' mot{i} = mot{i}/sqrt(sum(sum(mot{i}.^2))); */
  ...
  *ind = 1.0;
  ...
  for (b_i = 0; b_i <= i - 2; b_i++) {
    ...
    /* 'mapTF:946' rmat = rot90(mot{1},2)*mot{j}'; */
    ...
    /* 'mapTF:947' diag_a = sum(spdiags(mat)); */
    spdiags(mat, A);
    b_sum(A, diag_a);
    mat_data = diag_a->data;

    /* 'mapTF:948' diag_b = sum(spdiags(rmat)); */
    spdiags(rmat, A);
    b_sum(A, diag_b);
    rmat_data = diag_b->data;

    /* 'mapTF:949' diag_overall = zeros(length(diag_a)+length(diag_b),1); */
    ...
    /* 'mapTF:950' diag_overall(1:length(diag_a)) = diag_a; */
    loop_ub = diag_a->size[1];
    for (i1 = 0; i1 < loop_ub; i1++) {
      diag_overall_data[i1] = mat_data[i1];
    }

    /* 'mapTF:951' diag_overall(length(diag_a)+1:end) = diag_b; */
  ...
  *lfn, const emxArray_char_T *memefn, const emxArray_char_T *ofn)
{
  ...
  /* 'mapTF:690' LEN = zeros(1,1); */
  /* 'mapTF:691' shift = zeros(1,1); */
  /* 'mapTF:692' [p,w] = getdenovomotif(dfn); */
  ...
  /* 'mapTF:707' curr_pos = ftell(fid); */
  curr_pos = b_ftell(fileid);

  /* 'mapTF:708' idx=0; */
  idx = 0.0;

  /* 'mapTF:709' while ~feof(fid) */
  ...
  /* 'mapTF:713' fseek(fid, curr_pos, 'bof'); */
  b_fseek(fileid, curr_pos);

  /* 'mapTF:714' clus = zeros(idx, 1); */
  match_idx = (int)idx;
  i = clus->size[0];
  clus->size[0] = (int)idx;
  ...
  /* 'mapTF:716' lasso_weight = zeros(idx, 1); */
  ...
  while ((!exitg2) && (cur_idx <= (int)idx - 1)) {
    /* 'mapTF:720' cur_line = fgetl(fid); */
    fgetl(fileid, cur_line);

    /* 'mapTF:721' if cur_line == -1 */
    ...
    if (hal) {
      ...
      /* 'mapTF:724' [p1, remain] = strtok(cur_line, char(9)); */
      c_strtok(cur_line, p1, remain);

      /* 'mapTF:725' [p2, remain] = strtok(remain, char(9)); */
      c_strtok(remain, p2, b_remain);

      /* 'mapTF:726' [p3, remain] = strtok(remain, char(9)); */
      c_strtok(b_remain, cur_line, remain);

      /* 'mapTF:727' [p4, remain] = strtok(remain, char(9)); */
      c_strtok(remain, p4, b_remain);

      /* 'mapTF:728' [p5, p6] = strtok(remain, char(9)); */
      c_strtok(b_remain, cur_line, remain);

      /* 'mapTF:729' clus(cur_idx, 1) = real(str2double(p1)); */
      dc = str2double(p1);
      clus_data[cur_idx] = dc.re;

      /* 'mapTF:730' uid(cur_idx, 1) = real(str2double(p2)); */
      dc = str2double(p2);
      uid_data[cur_idx] = dc.re;

      /* 'mapTF:731' lasso_weight(cur_idx, 1) = real(str2double(p4)); */
      dc = str2double(p4);
      lasso_weight_data[cur_idx] = dc.re;

      /* 'mapTF:732' zscore(cur_idx, 1) = real(str2double(p5)); */
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:747' vec(i) = uid(f(1)); */
    vec_data[b_i] = uid_data[(int)f_data[0] - 1];

    /* 'mapTF:748' vec2(i) = zscore(f(1)); */
    vec2_data[b_i] = zscore_data[(int)f_data[0] - 1];

    /* 'mapTF:749' w(i+N) = lasso_weight(f(1)); */
  ...
  /* 'mapTF:751' P = getmotif(memefn,vec); */
  getmotif(memefn, vec, P);
  P_data = P->data;

  /* 'mapTF:753' denovo_len = length(p); */
  /* 'mapTF:754' database_len = length(P); */
  /* 'mapTF:755' cur_PWM = cell(denovo_len+database_len,1); */
  ...
  /* 'mapTF:756' cur_PWM = coder.nullcopy(cur_PWM); */
  /* 'mapTF:757' for cur_idx=1:denovo_len */
  i = cur_PWM_tmp->size[0];
  for (cur_idx = 0; cur_idx < i; cur_idx++) {
    /* 'mapTF:758' cur_PWM{cur_idx} = p{cur_idx}; */
  ...
  /* 'mapTF:760' for cur_idx=denovo_len+1:denovo_len+database_len */
  i = P->size[0];
  for (cur_idx = 0; cur_idx < i; cur_idx++) {
    b_cur_idx = ((unsigned int)cur_PWM_tmp->size[0] + cur_idx) + 1U;

    /* 'mapTF:761' cur_PWM{cur_idx} = P{cur_idx-denovo_len}; */
  ...
  /* 'mapTF:765' [pp, info, len] = trim_pwm(cur_PWM,0.25); */
  trim_pwm(cur_PWM, lasso_weight, uid);
  uid_data = uid->data;
  lasso_weight_data = lasso_weight->data;
  cur_PWM_data = cur_PWM->data;

  /* 'mapTF:767' coder.varsize("PWM2"); */
  /* 'mapTF:768' PWM2 = {pp{1}}; */
  ...
  /*  PWM2{1} = pp{1}; */
  /* 'mapTF:772' coder.varsize("LEN_2"); */
  /* 'mapTF:773' coder.varsize("I_2"); */
  /* 'mapTF:774' LEN_2 = zeros(1,1); */
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if (b_i + 1 > N) {
      ...
      /* 'mapTF:780' PWM2_len = length(PWM2); */
      ...
      /* 'mapTF:781' cur_PWM_tmp = cell(PWM2_len+1,1); */
      ...
      /* 'mapTF:784' for cur_idx=1:PWM2_len */
      for (cur_idx = 0; cur_idx < match_idx; cur_idx++) {
        /* 'mapTF:785' cur_PWM_tmp{cur_idx+1} = PWM2{cur_idx}; */
      ...
      /* 'mapTF:787' cur_length = zeros(length(LEN_2)+1,1); */
      ...
      /* 'mapTF:788' cur_length(1) = len(ii); */
      f_data[0] = uid_data[b_i];

      /* 'mapTF:789' cur_length(2:end) = LEN_2; */
      for (i1 = 0; i1 < match_idx; i1++) {
        f_data[i1 + 1] = vec_data[i1];
      }

      /* 'mapTF:790' [~,cor] = ppmsim(cur_PWM_tmp, cur_length); */
      ppmsim(cur_PWM_tmp, f, &curr_pos, &idx);

      /*  [~,cor] = ppmsim([pp{ii} PWM2], [len(ii) LEN_2]); */
      /* 'mapTF:792' if cor > 0.7 || vec2(ii-N) < 1.5 */
      if ((idx > 0.7) || (vec2_data[b_i - N] < 1.5)) {
        /* 'mapTF:793' hal = false; */
        hal = false;
      }
    } else if ((b_i + 1 > 1) && (a > 2.0)) {
      /* 'mapTF:795' elseif ii > 1 && a > 2 */
      /* 'mapTF:796' hal = true; */
      hal = true;

      /* 'mapTF:797' PWM2_len = length(PWM2); */
      match_idx = PWM2->size[0];
      if (match_idx < 1) {
        match_idx = 1;
      }
      ...
      /* 'mapTF:798' cur_PWM_tmp = cell(PWM2_len+1,1); */
      unnamed_idx_0 = match_idx + 1;
      ...
      /* 'mapTF:799' cur_PWM_tmp = coder.nullcopy(cur_PWM_tmp); */
      i1 = cur_PWM_tmp->size[0];
      cur_PWM_tmp->size[0] = b_cur_PWM_tmp->size[0];
      emxEnsureCapacity_cell_wrap_4(cur_PWM_tmp, i1);
      cur_PWM_tmp_data = cur_PWM_tmp->data;

      /* 'mapTF:800' cur_PWM_tmp{1} = pp{ii}; */
      i1 = cur_PWM_tmp_data[0].f1->size[0] * cur_PWM_tmp_data[0].f1->size[1];
      cur_PWM_tmp_data[0].f1->size[0] = cur_PWM_data[b_i].f1->size[0];
      cur_PWM_tmp_data[0].f1->size[1] = cur_PWM_data[b_i].f1->size[1];
      emxEnsureCapacity_real_T(cur_PWM_tmp_data[0].f1, i1);
      ...
      /* 'mapTF:801' for cur_idx=1:PWM2_len */
      for (cur_idx = 0; cur_idx < match_idx; cur_idx++) {
        /* 'mapTF:802' cur_PWM_tmp{cur_idx+1} = PWM2{cur_idx}; */
        i1 = cur_PWM_tmp_data[cur_idx + 1].f1->size[0] *
          cur_PWM_tmp_data[cur_idx + 1].f1->size[1];
        cur_PWM_tmp_data[cur_idx + 1].f1->size[0] = PWM2_data[cur_idx].f1->size
          [0];
      ...
      /* 'mapTF:804' cur_length = zeros(length(LEN_2)+1,1); */
      match_idx = LEN_2->size[0];
      if (match_idx < 1) {
        match_idx = 1;
      }
      ...
      /* 'mapTF:805' cur_length(1) = len(ii); */
      f_data[0] = uid_data[b_i];

      /* 'mapTF:806' cur_length(2:end) = LEN_2; */
      for (i1 = 0; i1 < match_idx; i1++) {
        f_data[i1 + 1] = vec_data[i1];
      }

      /* 'mapTF:807' [~,cor] = ppmsim(cur_PWM_tmp, cur_length); */
      ppmsim(cur_PWM_tmp, f, &curr_pos, &idx);

      /*  [~,cor] = ppmsim([pp{ii} PWM2], [len(ii) LEN_2]); */
      /* 'mapTF:809' if cor > 0.7 */
      if (idx > 0.7) {
        /* 'mapTF:810' hal = false; */
        hal = false;
      }
    ...
    /* 'mapTF:813' if hal && w(ii) > 0 && len(ii) >= 6 */
    if (hal && (w_data[b_i] > 0.0) && (uid_data[b_i] >= 6.0)) {
      /* 'mapTF:814' if len(ii) > 10 */
      if (uid_data[b_i] > 10.0) {
        /* 'mapTF:815' if info(ii)/len(ii) > 0.7 */
        if (lasso_weight_data[b_i] / uid_data[b_i] > 0.7) {
          /* 'mapTF:816' new_len = a+1; */
          /* 'mapTF:817' new_PWM2 = cell(new_len,1); */
          j = (int)(a + 1.0);
          ...
          /* 'mapTF:818' new_PWM2 = coder.nullcopy(new_PWM2); */
          i1 = cur_PWM_tmp->size[0];
          cur_PWM_tmp->size[0] = b_new_PWM2->size[0];
          emxEnsureCapacity_cell_wrap_4(cur_PWM_tmp, i1);
          cur_PWM_tmp_data = cur_PWM_tmp->data;

          /* 'mapTF:819' for idx = 1:new_len */
          for (match_idx = 0; match_idx < j; match_idx++) {
            /* 'mapTF:820' if idx <= a-1 */
            if ((double)match_idx + 1.0 <= a - 1.0) {
              /* 'mapTF:821' new_PWM2{idx} = PWM2{idx}; */
              ...
              /* 'mapTF:822' elseif idx == a */
              /* 'mapTF:823' new_PWM2{idx} = pp{ii}; */
              i1 = cur_PWM_tmp_data[match_idx].f1->size[0] *
                cur_PWM_tmp_data[match_idx].f1->size[1];
              ...
              /* 'mapTF:824' elseif idx == a+1 */
              /* 'mapTF:825' new_PWM2{a+1} = rot90(pp{ii},2); */
              rot90(cur_PWM_data[b_i].f1, cur_PWM_tmp_data[(int)(a + 1.0) - 1].
                    f1);
            }
          }

          /* 'mapTF:828' PWM2_len = length(new_PWM2); */
          /* 'mapTF:829' PWM2 = cell(PWM2_len, 1); */
          ...
          /* 'mapTF:830' PWM2 = coder.nullcopy(PWM2); */
          /* 'mapTF:831' for cur_idx = 1:PWM2_len */
          i1 = cur_PWM_tmp->size[0];
          for (cur_idx = 0; cur_idx < i1; cur_idx++) {
            /* 'mapTF:832' PWM2{cur_idx} = new_PWM2{cur_idx}; */
          ...
          /*  LEN_2(a) = LEN_2(a-1); */
          /*  I_2(a) = I_2(a-1); */
          /* 'mapTF:843' LEN_2_new = zeros(a+1,1); */
          i1 = f->size[0];
          f->size[0] = (int)(a + 1.0);
          emxEnsureCapacity_real_T(f, i1);
          f_data = f->data;
          ...
          /* 'mapTF:844' LEN_2_new(1:a-1) = LEN_2(1:a-1); */
          if (1.0 > a - 1.0) {
            loop_ub = 0;
          } else {
            loop_ub = (int)(a - 1.0);
          ...
          /* 'mapTF:845' LEN_2_new(a:a+1) = [len(ii), len(ii)]; */
          f_data[(int)a - 1] = uid_data[b_i];
          f_data[(int)(a + 1.0) - 1] = uid_data[b_i];

          /* 'mapTF:847' I_2_new = zeros(a+1,1); */
          i1 = clus->size[0];
          ...
          /* 'mapTF:848' I_2_new(1:a-1) = I_2(1:a-1); */
          if (1.0 > a - 1.0) {
            loop_ub = 0;
          } else {
            loop_ub = (int)(a - 1.0);
          ...
          a++;
                  ...
                  uid_data[b_i] > 1.0)) {
        /* 'mapTF:856' elseif info(ii) > 6 || info(ii)/len(ii) > 1 */
        /* 'mapTF:857' new_len = a+1; */
        /* 'mapTF:858' new_PWM2 = cell(new_len,1); */
        j = (int)(a + 1.0);
        unnamed_idx_0 = (int)(a + 1.0);
        i1 = new_PWM2->size[0];
        ...
        /* 'mapTF:859' new_PWM2 = coder.nullcopy(new_PWM2); */
        i1 = cur_PWM_tmp->size[0];
        cur_PWM_tmp->size[0] = new_PWM2->size[0];
        emxEnsureCapacity_cell_wrap_4(cur_PWM_tmp, i1);
        cur_PWM_tmp_data = cur_PWM_tmp->data;

        /* 'mapTF:860' for idx = 1:new_len */
        for (match_idx = 0; match_idx < j; match_idx++) {
          /* 'mapTF:861' if idx <= a-1 */
          if ((double)match_idx + 1.0 <= a - 1.0) {
            /* 'mapTF:862' new_PWM2{idx} = PWM2{idx}; */
            i1 = cur_PWM_tmp_data[match_idx].f1->size[0] *
              cur_PWM_tmp_data[match_idx].f1->size[1];
            cur_PWM_tmp_data[match_idx].f1->size[0] = PWM2_data[match_idx]
              .f1->size[0];
            ...
            /* 'mapTF:863' elseif idx == a */
            /* 'mapTF:864' new_PWM2{idx} = pp{ii}; */
            i1 = cur_PWM_tmp_data[match_idx].f1->size[0] *
              cur_PWM_tmp_data[match_idx].f1->size[1];
            ...
            /* 'mapTF:865' elseif idx == a+1 */
            /* 'mapTF:866' new_PWM2{a+1} = rot90(pp{ii},2); */
            rot90(cur_PWM_data[b_i].f1, cur_PWM_tmp_data[(int)(a + 1.0) - 1].f1);
          }
        }

        /* 'mapTF:869' PWM2_len = length(new_PWM2); */
        /* 'mapTF:870' PWM2 = cell(PWM2_len, 1); */
        match_idx = cur_PWM_tmp->size[0];
        i1 = PWM2->size[0] * PWM2->size[1];
        PWM2->size[0] = cur_PWM_tmp->size[0];
        PWM2->size[1] = 1;
        ...
        /* 'mapTF:871' PWM2 = coder.nullcopy(PWM2); */
        /* 'mapTF:872' for cur_idx = 1:PWM2_len */
        i1 = cur_PWM_tmp->size[0];
        for (cur_idx = 0; cur_idx < i1; cur_idx++) {
          /* 'mapTF:873' PWM2{cur_idx} = new_PWM2{cur_idx}; */
          match_idx = PWM2_data[cur_idx].f1->size[0] * PWM2_data[cur_idx]
            .f1->size[1];
        ...
        /*  LEN_2(a) = LEN_2(a-1); */
        /*  I_2(a) = I_2(a-1); */
        /* 'mapTF:884' LEN_2_new = zeros(a+1,1); */
        i1 = f->size[0];
        f->size[0] = (int)(a + 1.0);
        emxEnsureCapacity_real_T(f, i1);
        f_data = f->data;
        ...
        /* 'mapTF:885' LEN_2_new(1:a-1) = LEN_2(1:a-1); */
        if (1.0 > a - 1.0) {
          loop_ub = 0;
        } else {
          loop_ub = (int)(a - 1.0);
        ...
        /* 'mapTF:886' LEN_2_new(a:a+1) = [len(ii), len(ii)]; */
        f_data[(int)a - 1] = uid_data[b_i];
        f_data[(int)(a + 1.0) - 1] = uid_data[b_i];

        /* 'mapTF:888' I_2_new = zeros(a+1,1); */
        i1 = clus->size[0];
        clus->size[0] = (int)(a + 1.0);
        emxEnsureCapacity_real_T(clus, i1);
        clus_data = clus->data;
        ...
        /* 'mapTF:889' I_2_new(1:a-1) = I_2(1:a-1); */
        if (1.0 > a - 1.0) {
          loop_ub = 0;
        } else {
          loop_ub = (int)(a - 1.0);
        ...
        /* 'mapTF:890' I_2_new(a:a+1) = [info(ii), info(ii)]; */
        clus_data[(int)a - 1] = lasso_weight_data[b_i];
        clus_data[(int)(a + 1.0) - 1] = lasso_weight_data[b_i];

        /* 'mapTF:892' LEN_2 = LEN_2_new; */
        i1 = LEN_2->size[0] * LEN_2->size[1];
        LEN_2->size[0] = f->size[0];
        LEN_2->size[1] = 1;
        emxEnsureCapacity_real_T(LEN_2, i1);
  ...
  /* 'mapTF:899' num2 = length(strfind(fileread(memefn),'MOTIF')); */
  ...
  /* 'mapTF:900' [p,names] = getmotif(memefn,1:num2); */
  emxInit_real_T(&y, 2);
  if (match_idx < 1) {
    y->size[0] = 1;
    y->size[1] = 0;
  ...
  /* 'mapTF:901' [p,info,lenvec] = trim_pwm(p,0.25); */
  trim_pwm(cur_PWM_tmp, lasso_weight, f);
  f_data = f->data;
  cur_PWM_tmp_data = cur_PWM_tmp->data;

  /* 'mapTF:902' fid = fopen([ofn '_motifs.out'], 'w'); */
  ...
  for (b_i = 0; b_i < match_idx; b_i++) {
    /* 'mapTF:908' p_len = length(p); */
    /* 'mapTF:909' cur_PWM_tmp = cell(p_len+1,1); */
    ...
    /* 'mapTF:912' for cur_idx=1:p_len */
    for (cur_idx = 0; cur_idx < i2; cur_idx++) {
      /* 'mapTF:913' cur_PWM_tmp{cur_idx+1} = p{cur_idx}; */
    ...
    /* 'mapTF:915' [ind, r] = ppmsim(cur_PWM_tmp, [LEN_2(i);lenvec]); */
    i = clus->size[0];
    ...
    if (idx > 0.8) {
      ...
      if (I_2_data[b_i] / d > 1.0) {
        /* 'mapTF:924' elseif I_2(i)/LEN_2(i) > 1 */
        /* 'mapTF:925' fprintf(fid,'MOTIF %d\n%s\n%d\n', int32(a), consen(PWM2{i}, LEN_2(i)), int32(LEN_2(i))); */
        b_NULL = NULL;
        getfilestar(fileid, &filestar, &hal);
        if (!(filestar == b_NULL)) {
        ...
        for (j = 0; j < i; j++) {
          /* 'mapTF:928' fprintf(fid,'%0.3f %0.3f %0.3f %0.3f\n',PWM2{i}(j,1),PWM2{i}(j,2),PWM2{i}(j,3),PWM2{i}(j,4)); */
          print_processing(PWM2_data[b_i].f1->data[j], PWM2_data[b_i].f1->data[j
  ...
  emxArray_real_T *omat)
{
  ...
  /* ofn: output header */
  /* 'mapTF:273' L = length(ss); */
  L = ss->size[1];

  /* 'mapTF:274' nvar = numel(dsvm); */
  /* 'mapTF:275' loc = zeros(nvar, 1); */
  i = loc->size[0];
  ...
  /* 'mapTF:276' varvec = zeros(nvar, 1); */
  i = varvec->size[0];
  varvec->size[0] = dsvm->size[0];
  emxEnsureCapacity_int8_T(varvec, i);
  varvec_data = varvec->data;

  /* 'mapTF:277' varc = cell(4,1); */
  /* 'mapTF:278' varc{1} = [2 3 4]; */
  /* 'mapTF:279' varc{2} = [1 3 4]; */
  /* 'mapTF:280' varc{3} = [1 2 4]; */
  /* 'mapTF:281' varc{4} = [1 2 3]; */
  varc[0].f1[0] = 2.0;
  varc[1].f1[0] = 1.0;
  varc[2].f1[0] = 1.0;
  varc[3].f1[0] = 1.0;
  ...
  /* 'mapTF:282' for i = 1:nvar */
  i = dsvm->size[0];
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:283' loc(i) = floor((i-1)/3)+l; */
    loc_data[b_i] = floor((((double)b_i + 1.0) - 1.0) / 3.0) + l;

    /* 'mapTF:284' varvec(i) = varc{ss(loc(i))}(mod(i-1,3)+1); */
  ...
  /* 'mapTF:288' path_ref = mat(:,2); */
  /* 'mapTF:289' path_ref = [path_ref; zeros(l,1)]; */
  dimSize = mat->size[0];
  i = path_ref->size[0];
  ...
  /*  c = combnk(1:l,k); */
  /* 'mapTF:292' c = flip(nchoosek(1:l,k)); */
  /* 'mapTF:293' [c1,~] = size(c); */
  /* 'mapTF:294' O = ones(1,k); */
  /* 'mapTF:295' evec = zeros(nvar, 1); */
  i = evec->size[0];
  ...
  /* 'mapTF:297' GCvec = PWM(f(1),:); */
  /* 'mapTF:298' for i = 1:nvar */
  i = dsvm->size[0];
  emxInit_real_T(&vec, 2);
  vec_data = vec->data;
  ...
  emxInit_real_T(&a, 2);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:299' if lab(loc(i)) > 0 */
    i1 = (int)loc_data[b_i] - 1;
    d = lab_data[i1];
    if (d > 0.0) {
      /* 'mapTF:300' vec = (max([1 loc(i)-l+1]):min([L loc(i)+l-1])); */
      ...
      /* 'mapTF:301' LAB = lab(loc(i)); */
      /* 'mapTF:302' path_vec = path_ref(vec); */
      /* 'mapTF:303' path_loc = path_ref(loc(i)); */
      /* 'mapTF:304' scores = zeros(length(vec),1); */
      i2 = scores->size[0];
      ...
      /* 'mapTF:305' V = exp(PWM(loc(i),varvec(i)))-exp(PWM(loc(i), ss(loc(i)))); */
      /* 'mapTF:306' for j = 1:length(vec) */
      i2 = vec->size[1];
      for (j = 0; j < i2; j++) {
        /* 'mapTF:307' if lab(vec(j)) == LAB && path_loc-path_vec(j)==loc(i)-vec(j) */
        ...
        if ((lab_data[i3] == d) && (path_ref_data[i1] - path_ref_data[(int)
             ...
             vec_data[j])) {
          /* 'mapTF:308' scores(j) = PWM(vec(j),ss(vec(j))); */
          scores_data[j] = PWM_data[i3 + PWM->size[0] * ((int)ss_data[i3] - 1)];
        } else {
          /* 'mapTF:309' else */
          /* 'mapTF:310' scores(j) = GCvec(ss(vec(j))); */
          scores_data[j] = PWM_data[(f_data[0] + PWM->size[0] * ((int)ss_data[i3]
            - 1)) - 1];
        }
      }

      /* 'mapTF:313' if loc(i)-l+1 < 1 */
      if (x_idx_1_tmp < 1.0) {
        /* 'mapTF:314' a = l-loc(i); */
        b_a = l - loc_data[b_i];

        /* 'mapTF:315' scores(loc(i)) = 0; */
        scores_data[i1] = 0.0;
      } else {
        /* 'mapTF:316' else */
        /* 'mapTF:317' a = 0; */
        ...
        /* 'mapTF:318' scores(l) = 0; */
        scores_data[(int)l - 1] = 0.0;
      }

      /* 'mapTF:321' if loc(i)+l-1 > L */
      if (b_x_idx_1_tmp > L) {
        /* 'mapTF:322' b = l-1+L-loc(i); */
      ...
      for (j = 0; j < i2; j++) {
        ...
        /* 'mapTF:327' evec(i) = evec(i)+sum(exp(Smat{l-j}*scores(1+j-a:l+j-a))); */
  ...
  /* 'mapTF:337' u = unique(lab); */
  unique_vector(lab, u);
  vec_data = u->data;

  /* 'mapTF:338' for i = 1:numel(u) */
  i = u->size[0];
  emxInit_real_T(&r2, 1);
  emxInit_real_T(&b, 1);
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if (vec_data[b_i] != 0.0) {
      ...
      /* 'mapTF:343' if numel(f) ~= numel(F) */
      if (f->size[0] != path_ref->size[0]) {
        /* 'mapTF:344' F = [F;numel(dsvm)]; */
        i1 = path_ref->size[0];
        i2 = path_ref->size[0];
        path_ref->size[0]++;
      ...
      for (j = 0; j < i1; j++) {
        ...
        /* 'mapTF:351' evec(a) = evec(a)*(evec(a)'*dsvm(a))/(evec(a)'*evec(a)); */
        i2 = y->size[0];
  ...
  /* 'mapTF:355' omat = [loc varvec evec]; */
                   ...
                   *seqout, emxArray_cell_wrap_2 *seq)
{
  ...
  /* 'mapTF:549' fseek(fid, curr_pos, 'bof'); */
  b_fseek(fileid, curr_pos);

  /* 'mapTF:550' sequences = cell(idx, 1); */
  n = (int)idx;
  i = sequences->size[0];
  ...
  /* 'mapTF:551' sequences = coder.nullcopy(sequences); */
  /* 'mapTF:552' alpha = zeros(idx, 1); */
  i = alpha->size[0];
  ...
  while ((!exitg2) && (n <= (int)idx - 1)) {
    /* 'mapTF:554' cur_line = fgetl(fid); */
    fgetl(fileid, cur_line);

    /* 'mapTF:555' if cur_line == -1 */
    y = (cur_line->size[1] != 0);
    if (y) {
    ...
    if (y) {
      ...
      /* 'mapTF:558' [cur_seq, cur_alpha] = strtok(cur_line, char(9)); */
      c_strtok(cur_line, cur_seq, cur_alpha);

      /* 'mapTF:559' alpha(cur_idx,1) = real(str2double(cur_alpha)); */
      dc = str2double(cur_alpha);
      alpha_data[n] = dc.re;

      /* 'mapTF:560' sequences{cur_idx} = (strip(cur_seq)); */
  ...
  /* 'mapTF:562' fclose(fid); */
  cfclose(fileid);

  /* 'mapTF:564' l = length(sequences{1}); */
  varargin_2 = sequences_data[0].f1->size[1];

  /* 'mapTF:565' if l ~= input_l */
  ...
  /* 'mapTF:572' fprintf('calculating indices\n'); */
  printf("calculating indices\n");
  fflush(stdout);

  /* 'mapTF:573' for i = 1:numel(alpha) */
  i = alpha->size[0];
  emxInit_real_T(&ss, 2);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:574' ss = letterconvert(sequences{i}); */
    letterconvert(sequences_data[b_i].f1, ss);
    ss_data = ss->data;

    /* 'mapTF:575' rs = 3-fliplr(ss); */
    /* 'mapTF:576' w(ss*pow+1) = alpha(i); */
    if (ss->size[1] < 1) {
      curr_pos = 0.0;
    } else {
  ...
  /* 'mapTF:579' m = mean(alpha); */
  curr_pos = blockedSummation(alpha, alpha->size[0]) / (double)alpha->size[0];

  /* 'mapTF:580' s = std(alpha); */
  idx = b_std(alpha);

  /* 'mapTF:581' W = (1/2)*(1+erf((w-m)/s/sqrt(2))); */
  i = p->size[0];
  p->size[0] = w->size[0];
  ...
  /* 'mapTF:595' fseek(fid, curr_pos, 'bof'); */
  b_fseek(fileid, curr_pos);

  /* 'mapTF:596' seq = cell(idx, 1); */
  n = (int)idx;
  i = seq->size[0];
  seq->size[0] = (int)idx;
  ...
  /* 'mapTF:597' seq = coder.nullcopy(seq); */
  /* 'mapTF:598' for cur_idx=1:idx */
  n = 0;
  exitg2 = false;
  while ((!exitg2) && (n <= (int)idx - 1)) {
    /* 'mapTF:599' cur_line = fgetl(fid); */
    fgetl(fileid, cur_line);

    /* 'mapTF:600' if cur_line == -1 */
    y = (cur_line->size[1] != 0);
    if (y) {
      y = (0 > cur_line->size[1] - 1);
    }
    ...
    if (y) {
      ...
      /* 'mapTF:603' seq{cur_idx} = (strip(cur_line)); */
  ...
  /* 'mapTF:605' fclose(fid); */
  cfclose(fileid);

  /* 'mapTF:607' n = length(seq)/2; */
  curr_pos = (double)seq->size[0] / 2.0;

  /* 'mapTF:608' seqout = cell(n,1); */
  /* 'mapTF:609' seqindmat = cell(n,1); */
  b_j1 = (int)curr_pos;
  i = seqindmat->size[0];
  ...
  /* 'mapTF:610' seqindmat = coder.nullcopy(seqindmat); */
  /* 'mapTF:611' fprintf('converting kmers to probabilities\n'); */
  printf("converting kmers to probabilities\n");
  fflush(stdout);

  /* 'mapTF:612' P = cell(n,1); */
  /* 'mapTF:613' for i = 1:n */
  i = seqout->size[0];
  ...
  for (b_i = 0; b_i < b_j1; b_i++) {
    /* 'mapTF:614' if mod(i,1000)==0 */
    if (fmod((double)b_i + 1.0, 1000.0) == 0.0) {
      /* 'mapTF:615' fprintf('%d sequences converted\n', int32(i)); */
      printf("%d sequences converted\n", b_i + 1);
      fflush(stdout);
    }

    /* 'mapTF:617' L = length(seq{2*i})-l+1; */
    n = ((b_i + 1) << 1) - 1;
    idx = (double)(seq_data[n].f1->size[1] - varargin_2) + 1.0;

    /* 'mapTF:618' seqindmat{i} = zeros(L,1); */
    nd2 = (int)idx;
    ...
    /* 'mapTF:619' ss = letterconvert(seq{2*i}); */
    letterconvert(seq_data[n].f1, ss);
    ss_data = ss->data;

    /* 'mapTF:620' seqout{i} = ss+1; */
  ...
  /* 'mapTF:632' fprintf('Running dsvm\n') */
  printf("Running dsvm\n");
  fflush(stdout);

  /* 'mapTF:633' mat = [1 2 3;0 2 3;0 1 3;0 1 2]; */
  /* 'mapTF:634' O = ones(1,l); */
  i = O->size[0] * O->size[1];
  O->size[0] = 1;
  ...
  emxInit_real_T(&S, 2);
  for (b_i = 0; b_i < b_j1; b_i++) {
    /* 'mapTF:637' if mod(i,1000)==0 */
    if (fmod((double)b_i + 1.0, 1000.0) == 0.0) {
      /* 'mapTF:638' fprintf('%d sequences converted\n', int32(i)); */
      printf("%d sequences converted\n", b_i + 1);
      fflush(stdout);
    }

    /* 'mapTF:640' L = length(seq{2*i})-2*l+2; */
    n = ((b_i + 1) << 1) - 1;
    idx = ((double)seq_data[n].f1->size[1] - 2.0 * (double)varargin_2) + 2.0;

    /* 'mapTF:641' ss = letterconvert(seq{2*i}); */
    letterconvert(seq_data[n].f1, ss);
    ss_data = ss->data;

    /* 'mapTF:642' p = zeros(L+l-1,1); */
    ...
    /* 'mapTF:645' for j = 2:L+l-1 */
    i = (int)(((idx + (double)sequences_data[0].f1->size[1]) - 1.0) + -1.0);
    for (j = 0; j < i; j++) {
      /* 'mapTF:646' I = (I-ss(j-1))/4+4^(l-1)*ss(j+l-1); */
      b_I = (b_I - ss_data[j]) / 4.0 + rt_powd_snf(4.0, (double)varargin_2 - 1.0)
    ...
    for (j = 0; j <= i; j++) {
      ...
      /* 'mapTF:654' for ii = 1:3 */
      for (loop_ub = 0; loop_ub < 3; loop_ub++) {
        /* 'mapTF:655' S(l) = mat(ss(j)+1,ii); */
        S_data[varargin_2 - 1] = mat[((int)(ss_data[(int)b_j - 1] + 1.0) +
          (loop_ub << 2)) - 1];

        /* 'mapTF:656' I = S(1:l)*pow; */
                     ...
                     emxArray_real_T *len)
{
  ...
  /* 'mapTF:1054' l = length(p); */
  /* 'mapTF:1055' info = zeros(l, 1); */
  nx = p->size[0];
  i = info->size[0];
  info->size[0] = nx;
  ...
  /* 'mapTF:1056' len = zeros(l,1); */
  i = len->size[0];
  len->size[0] = nx;
  emxEnsureCapacity_real_T(len, i);
  len_data = len->data;

  /* 'mapTF:1057' for i = 1:l */
  i = p->size[0];
  emxInit_real_T(&mat, 2);
  emxInit_real_T(&vec, 1);
  emxInit_real_T(&x, 2);
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    /* 'mapTF:1059' vec = 2+sum(mat.*log(mat)/log(2),2); */
    ...
    /* 'mapTF:1060' while (vec(1) < cut || mean(vec(1:3)) < cut || mean(vec(2:4)) < cut) && length(vec) > 4 */
    ...
    /* 'mapTF:1064' while (vec(end) < cut || mean(vec(end-2:end)) < cut || mean(vec(end-3:end-1)) < cut) && length(vec) > 4 */
    ...
    /* 'mapTF:1068' info(i) = sum(vec); */
    info_data[b_i] = blockedSummation(vec, vec->size[0]);

    /* 'mapTF:1069' [len(i), ~] = size(p{i}); */
 ...
 */
void mapTF(const emxArray_char_T *varargin_1, const emxArray_char_T
               ...
               varargin_8, double varargin_9)
{
  ...
  /*  end */
  /* 'mapTF:6' fn = varargin{1}; */
  /* 'mapTF:7' wfn = varargin{2}; */
  /* 'mapTF:8' mfn1 = varargin{3}; */
  /* 'mapTF:9' mfn2 = varargin{4}; */
  /* 'mapTF:10' memefn = varargin{5}; */
  /* 'mapTF:11' ofn = varargin{6}; */
  /* 'mapTF:12' l_svm = varargin{7}; */
  /* 'mapTF:13' k_svm = varargin{8}; */
  /* 'mapTF:14' nfrac = varargin{9}; */
  /* 'mapTF:16' if l_svm < k_svm */
  ...
  /*      end */
  /*  end */
  /* 'mapTF:28' fprintf('Processing Motifs\n'); */
  printf("Processing Motifs\n");
  fflush(stdout);

  /* 'mapTF:29' process_motifs(mfn1, mfn2, memefn, ofn) */
  process_motifs(varargin_3, varargin_4, varargin_5, varargin_6);

  /* 'mapTF:30' mfn = [ofn '_motifs.out']; */
  /* 'mapTF:31' [P,V, seqindmat, ss, seq] = seq2pv(fn, wfn, l_svm); */
  ...
  /* 'mapTF:672' for i = 1:length(s) */
  i = ss->size[0];
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:673' S = s{i}; */
    /* 'mapTF:674' N = length(S); */
    /* 'mapTF:675' L = N + L; */
  ...
  /* 'mapTF:33' GCmat = [0.5-GC/2 GC/2 GC/2 0.5-GC/2]; */
  L = 0.5 - GC / 2.0;
  GCmat[0] = L;
  GCmat[1] = GC / 2.0;
  GCmat[2] = GC / 2.0;
  ...
  /* 'mapTF:35' [p,names,len] = getMOTIF(mfn); */
  ...
  /* 'mapTF:36' a = numel(len); */
  /* 'mapTF:37' PWM = cell(a,1); */
  nx = len->size[0];
  i = PWM->size[0];
  ...
  /* 'mapTF:38' PWM = coder.nullcopy(PWM); */
  /* 'mapTF:39' PWM2 = cell(a,1); */
  i = PWM2->size[0];
  ...
  /* 'mapTF:40' PWM2 = coder.nullcopy(PWM2); */
  /* 'mapTF:42' LEN = zeros(a,1); */
  i = LEN->size[0];
  ...
  /* 'mapTF:43' LEN_2 = zeros(a,1); */
  /* 'mapTF:44' shift = zeros(a,1); */
  /* 'mapTF:45' for i = 1:a */
  i = len->size[0];
  i1 = shift->size[0];
  ...
  emxInit_real_T(&r1, 2);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:46' shift(i) = max([l_svm-len(i) 4]); */
    ...
    /* 'mapTF:47' PWM{i} = [repmat(GCmat,shift(i), 1); p{i} ;repmat(GCmat,shift(i), 1)]; */
    repmat(GCmat, d, r);
    all_pwm_data = r->data;
    repmat(GCmat, d, r1);
    ...
    /* 'mapTF:49' LEN_2(i) = len(i); */
    LEN_2_data[b_i] = f_data[b_i];

    /* 'mapTF:50' LEN(i) = len(i)+2*shift(i)-l_svm+1; */
    d = ((f_data[b_i] + 2.0 * shift_data[b_i]) - varargin_7) + 1.0;
    LEN_data[b_i] = d;

    /* 'mapTF:51' for j = 1:LEN(i) */
    i1 = (int)d;
    for (j = 0; j < i1; j++) {
      /* 'mapTF:52' b = b+1; */
      GC++;
  ...
  /* 'mapTF:57' pwm = coder.nullcopy(pwm); */
  /* 'mapTF:58' lpwm = cell(b,1); */
  i = lpwm->size[0];
  lpwm->size[0] = (int)GC;
  ...
  /* 'mapTF:59' lpwm = coder.nullcopy(lpwm); */
  /* 'mapTF:60' lab = zeros(b,1); */
  /* 'mapTF:61' b = 1; */
  GC = 1.0;
  ...
  i = len->size[0];
  for (b_i = 0; b_i < i; b_i++) {
    /* 'mapTF:63' for j = 1:LEN(i) */
    i1 = (int)LEN_data[b_i];
    for (j = 0; j < i1; j++) {
      /* 'mapTF:64' pwm{b} = PWM{i}(j:j+l_svm-1,:); */
      ...
      /* 'mapTF:65' lpwm{b} = log((pwm{b}+10^-10)/(1+4*10^-10)); */
  ...
  /* 'mapTF:74' c = [c;l_svm+1-fliplr(c2)]; */
  j = seqmat->size[1];
  ...
  /* 'mapTF:75' C = numel(c)/k_svm; */
  L = (double)(c->size[0] * c->size[1]) / varargin_8;

  /* 'mapTF:76' seqmat = zeros(C,l_svm); */
  i = (int)L;
  ...
  /* 'mapTF:81' Smat = coder.nullcopy(Smat); */
  /* 'mapTF:82' for i = 1:l_svm */
  emxInit_boolean_T(&b_f, 1);
  for (b_i = 0; b_i < nx; b_i++) {
    /* 'mapTF:83' f = find(prod(c-i,2)==0); */
    i = pwm_prob->size[0] * pwm_prob->size[1];
    ...
    /* 'mapTF:84' Smat{i} = zeros(length(f), l_svm); */
    ...
    /* 'mapTF:85' for j = 1:length(f) */
    i = f->size[0];
    for (j = 0; j < i; j++) {
      /* 'mapTF:86' Smat{i}(j,c(f(j),:)) = 1; */
      loop_ub = c->size[1];
      i1 = r2->size[0];
      r2->size[0] = c->size[1];
      emxEnsureCapacity_int32_T(r2, i1);
  ...
  /* 'mapTF:89' L = length(ss); */
  /* 'mapTF:90' B = b-1; */
  /* 'mapTF:91' maxnorm = zeros(B,1); */
  /* 'mapTF:92' minnorm = zeros(B,1); */
  /* 'mapTF:93' vec = zeros(l_svm,1); */
  /* 'mapTF:94' IND = zeros(4^l_svm,1); */
  ...
  for (j = 0; j <= i - 2; j++) {
    /* 'mapTF:97' vec = max(lpwm{j}'); */
    /* 'mapTF:98' vec2 = min(lpwm{j}'); */
    /* 'mapTF:99' maxnorm(j) = sum(exp(seqmat*vec')); */
    ...
    /* 'mapTF:100' minnorm(j) = sum(exp(seqmat*vec2')); */
    i1 = b_lpwm->size[0] * b_lpwm->size[1];
    b_lpwm->size[0] = 4;
    loop_ub = PWM2_data[j].f1->size[0];
    b_lpwm->size[1] = PWM2_data[j].f1->size[0];
  ...
  /* 'mapTF:102' dnorm = maxnorm-minnorm; */
  ...
  /* 'mapTF:103' vec = zeros(l_svm,1); */
  /* 'mapTF:105' all_pwm = zeros(4, l_svm, b-1); */
  ...
  /* 'mapTF:106' for cur_idx=1:b-1 */
  for (sizes_idx_0 = 0; sizes_idx_0 <= i - 2; sizes_idx_0++) {
    /* 'mapTF:107' all_pwm(:,:,cur_idx) = lpwm{cur_idx}'; */
  ...
  /* 'mapTF:110' fprintf('Mapping motifs\n'); */
  ...
  for (b_I = 0; b_I < i1; b_I++) {
    /* 'mapTF:113' seq2 = ss{I}; */
    /* 'mapTF:115' ss_onehot = zeros(4, length(seq2)); */
    ...
    /* 'mapTF:121' for i = 1:length(seqindmat{I}) */
    i2 = seqindmat_data[b_I].f1->size[0];
    for (b_i = 0; b_i < i2; b_i++) {
      /* 'mapTF:122' ind = seqindmat{I}(i); */
      /* 'mapTF:123' if IND(ind) == 0 */
      ...
      if (IND_data[i3] == 0) {
        ...
        /*              end */
        /* 'mapTF:132' seq3 = ss_onehot(:,i:i+l_svm-1); */
        ...
        /* 'mapTF:133' vec3 = reshape(nonzeros(all_pwm .* seq3), l_svm, b-1); */
        ...
        for (j = 0; j <= i - 2; j++) {
          /* 'mapTF:135' kmat(j,ind) = sum(exp(seqmat*vec3(:,j))); */
          i4 = b_B->size[0];
          b_B->size[0] = i6;
        ...
        /* 'mapTF:139' kmat(:,ind) = log((kmat(:,ind)-minnorm)./dnorm); */
        loop_ub = c->size[0];
        if (c->size[0] == 1) {
          j = minnorm->size[0];
        } else {
    ...
    /* 'mapTF:145' MAPTF(fn, mfn, seq{I*2}, ss{I}, GC, pwm_prob, Smat, l_svm, k_svm, ofn, PWM, PWM2, pwm, lpwm, lab, LEN, LEN_2, shift, P{I}, V{I}, names, len, a,b, I) */
    i2 = f->size[0];
    f->size[0] = LEN->size[0];
    emxEnsureCapacity_real_T(f, i2);
    f_data = f->data;
    ...
    /* 'mapTF:146' if mod(I,100)==0 */
    if (b_mod((double)b_I + 1.0, 100.0) == 0.0) {
      /* 'mapTF:147' fprintf('%d out of %d done...\n', int32(I), int32(length(ss))); */
