...
static void q_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *c,
                               unsigned int j, const emxArray_real_T *vec);

static void r_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *c,
                               int i, const emxArray_real_T *vec);

static void s_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *C,
...
static void t_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *c,
  ...
  for (i = 0; i < loop_ub; i++) {
    ...
    for (i1 = 0; i1 < b_loop_ub; i1++) {
      x_data[i1 + x->size[0] * i] =
          c_data[((int)b_r[i1 * stride_0_0] + c->size[0] * aux_0_1) - 1] -
                          ...
                          double Lfrac, emxArray_real_T *mat)
{
  ...
  while ((((int)count == 0) || (count < Lfrac)) &&
    ...
    while ((!exitg1) && (j <= (int)(M - 1.0) - 1)) {
      ...
      r2 = b_r->data;
      for (i = 0; i < nxin; i++) {
        c_r = cvec_data[i] + (((double)j + 1.0) - 1.0);
        r2[i] = b_mod(c_r, M);
      }
      i = b_r->size[0] * b_r->size[1];
      b_r->size[0] = 1;
      ...
      vlen = b_r->size[0] * b_r->size[1];
      b_r->size[0] = 1;
      nxin = b_k - i;
      b_r->size[1] = nxin;
      emxEnsureCapacity_real_T(b_r, vlen);
      r2 = b_r->data;
      for (b_k = 0; b_k < nxin; b_k++) {
        c_r = ((l + 1.0) - cvec_data[i + b_k]) + (((double)j + 1.0) - 1.0);
        r2[b_k] = b_mod(c_r, M);
      }
      i = b_r->size[0] * b_r->size[1];
      b_r->size[0] = 1;
  ...
  for (b_i = 0; b_i <= loop_ub - 2; b_i++) {
    ...
    if (d_data[b_i] != 0.0) {
      ...
      if (c->size[1] == vec->size[1]) {
        ...
        r_binary_expand_op(x, c, b_i, vec);
        x_data = x->data;
      }
      nx = x->size[1];
      i = y->size[0] * y->size[1];
      ...
      if (sum(y) == 0.0) {
        ...
        for (j = 0; j <= i - 2; j++) {
          ...
          if (c->size[1] == vec->size[1]) {
            ...
            q_binary_expand_op(x, c, b_j, vec);
            x_data = x->data;
          }
          nx = x->size[1];
  ...
  *rcnum = a - 1.0;
  ...
  for (b_i = 0; b_i <= i - 2; b_i++) {
    ...
    for (j = 0; j <= i1 - 2; j++) {
      ...
      if (C->size[1] == x->size[1]) {
        ...
        s_binary_expand_op(x, C, f, d, b_i);
        x_data = x->data;
      }
      nx = x->size[1];
  ...
  for (b_i = 0; b_i < loop_ub_tmp; b_i++) {
    ...
    if (c->size[1] == C->size[1]) {
      ...
      t_binary_expand_op(x, c, b_ind, f, b_i, C);
      x_data = x->data;
    }
    nx = x->size[1];
    i = y->size[0] * y->size[1];
  ...
  if (n_frac != 1.0) {
    ...
    if (xtmp == 0.0) {
      ...
      if ((c2->size[0] == 0) || (c2->size[1] == 0)) {
        ...
        colMajorFlatIter(c2, c2->size[0], f);
      }
