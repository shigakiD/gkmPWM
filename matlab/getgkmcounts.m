function [gkmc, GCpos, GCneg, mat, mat2] = getgkmcounts(filename,l,k,lk,RC,comb,rcnum)
...
if RC
    ...
    x = encodekmers_norc(l, k, comb);
end  
if length(lk) > 1
    disp('Using mismatches to calculate norms');
    ll = lk(1);
    kk = lk(2);
    LK = 0;
    Ker = zeros(ll+1,1);
    for i = 0:(ll-kk)
        Ker(ll-i+1) = nchoosek(ll-i,ll-kk-i);
    end
else
...
for i = 1:n
    ...
    if LK == 1
        ...
        for ii = 1:4
            for j = 1:2
                C{ii,j} = zeros(len(i)-ll+1, ll);
            end
        end
        for ii = 1:len(i)
            for j = 0:ll-1
                if ii - j <= len(i)-ll+1 && ii - j > 0
