function [B,stats] = lasso_cvmat(X,Y,varargin)
...
[weights, alpha, nLambda, lambdaRatio, lambda, ...
    dfmax, standardize, reltol, cvp, mcreps, maxIter,predictorNames, ParOptions] ...
...
function mse = lassoFitAndPredict(Xtrain,Ytrain,Xtest,Ytest, ...
    lambda,alpha,dfmax,standardize,reltol,ever_active,userSuppliedLambda,maxIter)
...
[B,Intercept] = lassoFit(Xtrain,Ytrain, ...
    trainWeights,lambda,alpha,dfmax,standardize,reltol,lambdaMax,ever_active,userSuppliedLambda,nullMSE,maxIter);
Bplus = [Intercept; B];
testWeights = Xtest(:,1);
...
for i = 1:nLambda
    ...
    for numIter = 1:maxIter
        ...
        bold = b;
        [b,active] = cdescentCycle(XX0,wX0,XY0, ...
        ...
        if norm( (b-bold) ./ (1.0 + abs(bold)), Inf ) < reltol
            ...
            if any(potentially_active)
                ...
                [b,new_active] = cdescentCycle(XX0,wX0,XY0, ...
...
for j=find(active)
    ...
    bj = (XY0(j)-XX0(j,:)*b+XX0(j,j)*b(j)) / totalweight;
    % Soft thresholding
