...
static void
n_binary_expand_op(const emxArray_real_T *X, const emxArray_real_T *Y,
                   const double lambda_data[], int lambda_size[2], double dfMax,
                   double lambdaMax, const emxArray_real_T *y,
                   const emxArray_real_T *ex, const emxArray_real_T *b_y,
                   emxArray_real_T *B, double Intercept_data[],
                   int Intercept_size[2], double mse_data[], int mse_size[2]);

static void o_binary_expand_op(emxArray_boolean_T *okrows,
...
static void r_binary_expand_op(emxArray_boolean_T *ever_active,
                     ...
                     int mspe_size[2])
{
  ...
  if ((A->size[1] == y->size[1]) && (ever_active->size[1] == acoef)) {
    ...
    r_binary_expand_op(ever_active, A, y);
    ever_active_data = ever_active->data;
  }
  emxInit_real_T(&Y0, 1);
    ...
    if (lambda_data[b_i] >= lambdaMax) {
      ...
      while ((!exitg2) && (numIter < 100000)) {
        ...
        if ((b_b->size[0] == bold->size[0]) && (acoef == fit->size[0])) {
          ...
          q_binary_expand_op(bold, b_b, fit);
          shrinkFactor_data = bold->data;
        }
        if (bold->size[0] == 0) {
          a = 0.0;
                   ...
                   int Intercept_size[2], double mse_data[], int mse_size[2])
{
  ...
  for (i = 0; i < loop_ub; i++) {
    b_y_data[i] = (y_data[i * stride_0_1] - ex_data[i * stride_1_1] != 0.0);
  }
  lassoFit(X, Y, lambda_data, lambda_size, dfMax, lambdaMax, c_y,
           blockedSummation(b_y, b_y->size[0]) / (double)b_y->size[0], B,
           Intercept_data, Intercept_size, mse_data, mse_size);
...
/*
 * function [B,stats] = lasso_mat(X,Y,dfMax, Std)
 */
                 ...
                 double stats_MSE_data[], int stats_MSE_size[2])
{
  ...
  if (X->size[0] == Y->size[0]) {
    ...
    emxEnsureCapacity_boolean_T(okrows, k);
    okrows_data = okrows->data;
    npages = X->size[0];
    for (k = 0; k < npages; k++) {
      okrows_data[k] = true;
    ...
    o_binary_expand_op(okrows, outsize, b_outsize);
  }
  /*  -------------------------------------------------------------------- */
  ...
  for (k = 0; k <= nx; k++) {
    npages = b_r->size[0];
    for (n = 0; n < npages; n++) {
      dotp_data[n + b_X->size[0] * k] = X_data[(r1[n] + X->size[0] * k) - 1];
    }
  }
  emxFree_int32_T(&b_r);
  k = X->size[0] * X->size[1];
  ...
  X_data = X->data;
  npages = b_X->size[0] * b_X->size[1];
  for (k = 0; k < npages; k++) {
    X_data[k] = dotp_data[k];
  }
  ...
  muY = blockedSummation(Y, npages) / (double)npages;
  ...
  bsxfun(X, c_y, b_X);
  dotp_data = b_X->data;
  if ((b_X->size[0] == 0) || (b_X->size[1] == 0) || (Y0->size[0] == 0)) {
    k = b_y->size[0];
    b_y->size[0] = b_X->size[1];
    ...
    emxEnsureCapacity_real_T(b_y, k);
    y_data = b_y->data;
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, (blasint)b_X->size[1],
                (blasint)1, (blasint)b_X->size[0], 1.0, &dotp_data[0],
                (blasint)b_X->size[0], &Y0_data[0], (blasint)Y0->size[0], 0.0,
                &y_data[0], (blasint)b_X->size[1]);
  }
  emxFree_real_T(&b_X);
  ...
  if (d1 == -muY) {
    muY /= 99.0;
    for (k = 0; k < 98; k++) {
      b_y_data[k + 1] = (2.0 * ((double)k + 2.0) - 101.0) * muY;
    }
  } else if (((d1 < 0.0) != (muY < 0.0)) &&
             ((fabs(d1) > 8.9884656743115785E+307) ||
              (fabs(muY) > 8.9884656743115785E+307))) {
    delta1 = d1 / 99.0;
    muY /= 99.0;
    for (k = 0; k < 98; k++) {
      b_y_data[k + 1] =
          (d1 + muY * ((double)k + 1.0)) - delta1 * ((double)k + 1.0);
    }
  } else {
    delta1 = (muY - d1) / 99.0;
    for (k = 0; k < 98; k++) {
      b_y_data[k + 1] = d1 + ((double)k + 1.0) * delta1;
    }
  }
  lambda_size[0] = 1;
  lambda_size[1] = 100;
  ...
  if (X->size[1] >= 1) {
    for (npages = 0; npages < n; npages++) {
      y_data[npages] = X_data[X->size[0] * npages];
      for (i = 2; i <= nx; i++) {
        d1 = X_data[(i + X->size[0] * npages) - 1];
        if (y_data[npages] < d1) {
          y_data[npages] = d1;
        }
      }
  ...
  if (X->size[1] >= 1) {
    for (npages = 0; npages < n; npages++) {
      dotp_data[npages] = X_data[X->size[0] * npages];
      for (i = 2; i <= nx; i++) {
        d1 = X_data[(i + X->size[0] * npages) - 1];
        if (dotp_data[npages] > d1) {
          dotp_data[npages] = d1;
        }
      }
    }
  ...
  if (y->size[1] == c_y->size[1]) {
    ...
    d_y->size[0] = 1;
    d_y->size[1] = y->size[1];
    emxEnsureCapacity_boolean_T(d_y, k);
    okrows_data = d_y->data;
    npages = y->size[1];
    for (k = 0; k < npages; k++) {
      okrows_data[k] = (y_data[k] - dotp_data[k] != 0.0);
    }
    lassoFit(X, Y, lambda_data, lambda_size, dfMax, lambdaMax, d_y,
             blockedSummation(b_y, b_y->size[0]) / (double)b_y->size[0], b_B,
             Intercept_data, Intercept_size, mse_data, mse_size);
    dotp_data = b_B->data;
    emxFree_boolean_T(&d_y);
  } else {
    n_binary_expand_op(X, Y, lambda_data, lambda_size, dfMax, lambdaMax, y, c_y,
                       b_y, b_B, Intercept_data, Intercept_size, mse_data,
                       mse_size);
    dotp_data = b_B->data;
  }
  emxFree_real_T(&c_y);
  ...
  for (k = 0; k < npages; k++) {
    okrows_data[k] = (dotp_data[k] != 0.0);
  }
  nx = x->size[0];
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    npages = x->size[1];
    if (0 <= npages - 1) {
      memset(&nz_data[0], 0, npages * sizeof(int));
    }
  } else {
    npages = x->size[1];
    for (i = 0; i < npages; i++) {
      n = i * x->size[0];
      nz_data[i] = okrows_data[n];
      for (k = 2; k <= nx; k++) {
        nz_data[i] += okrows_data[(n + k) - 1];
      }
    }
  }
  emxFree_boolean_T(&x);
  ...
  for (k = 0; k < n; k++) {
    stats_Lambda_data[k] = lambda_data[reverseIndices_data[k] - 1];
  }
  ...
  for (k = 0; k < nx; k++) {
    for (n = 0; n < npages; n++) {
      Y_data[n + B->size[0] * k] =
          dotp_data[n + b_B->size[0] * (reverseIndices_data[k] - 1)];
    }
  }
  emxFree_real_T(&b_B);
  ...
  for (k = 0; k < nx; k++) {
    b_nz_data[k] = nz_data[reverseIndices_data[k] - 1];
  }
  if (0 <= nx - 1) {
    memcpy(&nz_data[0], &b_nz_data[0], nx * sizeof(int));
  }
  ...
  for (k = 0; k < nx; k++) {
    stats_Intercept_data[k] = Intercept_data[reverseIndices_data[k] - 1];
  }
  ...
  stats_DF_size[1] = nx;
  for (k = 0; k < nx; k++) {
    stats_DF_data[k] = nz_data[k];
  }
  /* 'lasso_cvmat:569' stats.MSE = mse; */
  stats_MSE_size[0] = 1;
  stats_MSE_size[1] = nx;
  for (k = 0; k < nx; k++) {
    stats_MSE_data[k] = mse_data[reverseIndices_data[k] - 1];
  }
  *stats_Alpha = 1.0;
}
