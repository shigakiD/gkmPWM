  ...
  for (b_i = 0; b_i <= loop_ub - 2; b_i++) {
    ...
    if (d_data[b_i] != 0.0) {
      ...
      if (c->size[1] == vec->size[1]) {
        ...
        f_binary_expand_op(x, c, b_i, vec);
        x_data = x->data;
      }
      nx = x->size[1];
      i = y->size[0] * y->size[1];
      ...
      if (c_sum(y) == 0.0) {
        ...
        for (j = 0; j <= i - 2; j++) {
          ...
          if (c->size[1] == vec->size[1]) {
            ...
            e_binary_expand_op(x, c, b_j, vec);
            x_data = x->data;
          }
          nx = x->size[1];
  ...
  *rcnum = a - 1.0;
  ...
  for (b_i = 0; b_i <= i - 2; b_i++) {
    ...
    for (j = 0; j <= i1 - 2; j++) {
      ...
      if (C->size[1] == x->size[1]) {
        ...
        g_binary_expand_op(x, C, f, d, b_i);
        x_data = x->data;
      }
      nx = x->size[1];
  ...
  for (b_i = 0; b_i < loop_ub_tmp; b_i++) {
    ...
    if (c->size[1] == C->size[1]) {
      ...
      h_binary_expand_op(x, c, b_ind, f, b_i, C);
      x_data = x->data;
    }
    nx = x->size[1];
    i = y->size[0] * y->size[1];
  ...
  if (n_frac != 1.0) {
    ...
    if (xtmp == 0.0) {
      ...
      if (rtIsNaN(l - 1.0)) {
        ...
        f->size[0] = 1;
        f->size[1] = 0;
      } else if (rtIsInf(l - 1.0) && (1.0 == l - 1.0)) {
        i = f->size[0] * f->size[1];
        f->size[0] = 1;
        f->size[1] = 1;
        emxEnsureCapacity_real_T(f, i);
      ...
      genIndex_frac(l - 1.0, k, b_c2,
                    rt_roundd_snf((L1 * (l - 1.0) * 2.0 - c_sum(r1)) / k / 2.0),
                    c2);
      c2_data = c2->data;
