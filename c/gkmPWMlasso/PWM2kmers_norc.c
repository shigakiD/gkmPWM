...
static void n_binary_expand_op(emxArray_real_T *kweig, int i1, int i2, int i3,
                    ...
                    emxArray_real_T *kweig)
{
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    for (i1 = 0; i1 < 4; i1++) {
      for (i2 = 0; i2 < 4; i2++) {
        b_k = i2 << 2;
        b_p[i1 + b_k] = ((p_data[b_i].f1[i1] * negmat[b_k] +
                          p_data[b_i].f1[i1 + 4] * negmat[b_k + 1]) +
                         p_data[b_i].f1[i1 + 8] * negmat[b_k + 2]) +
                        p_data[b_i].f1[i1 + 12] * negmat[b_k + 3];
      }
    }
    for (i1 = 0; i1 < 16; i1++) {
  ...
  loop_ub = (int)(n_tmp * (double)(c->size[0] * c->size[1]) / k);
  i = kweig->size[0];
      ...
      if ((b_i != M - 1.0) || (b_i2 + 1U <= (unsigned int)rx) ||
        ...
        for (i2 = 0; i2 < nrows; i2++) {
          for (b_k = 0; b_k < loop_ub; b_k++) {
            sPWM_data[b_k + sPWM->size[0] * i2] =
                mat_data[((int)indvec_data[i2] + mat->size[0] * b_k) - 1];
          }
        }
          ...
          if (indloc_data[(int)indvec_data[(int)b_i3 - 1] - 1] == 0.0) {
            ...
            if (indloc_data[(int)indvec_data[(int)(b_i3 - 1.0) - 1] - 1] ==
              ...
              for (ibcol = 0; ibcol < 4; ibcol++) {
                ...
                loop_ub = (c_i3 - b_k) - 1;
                c_i3 = b_ktree->size[0] * b_ktree->size[1];
                b_ktree->size[0] = 1;
                b_ktree->size[1] = loop_ub;
                emxEnsureCapacity_real_T(b_ktree, c_i3);
                a_data = b_ktree->data;
                for (c_i3 = 0; c_i3 < loop_ub; c_i3++) {
                  a_data[c_i3] =
                      ktree_data[(int)(b_i3 - 1.0) - 1].f1->data[c_i3] * matt;
                }
                loop_ub = b_ktree->size[1];
                for (c_i3 = 0; c_i3 < loop_ub; c_i3++) {
                  ktree_data[(int)b_i3 - 1].f1->data[(b_k + c_i3) + 1] =
                      a_data[c_i3];
                }
              }
            } else {
              ...
              if (b_r->size[0] == y->size[0]) {
                ...
                j_binary_expand_op(ktree, b_i3, b_r, y);
                ktree_data = ktree->data;
              }
            }
          } else {
            ...
            for (b_k = 0; b_k < loop_ub; b_k++) {
              nrows = ktree_data[(int)(b_i3 - 1.0) - 1].f1->size[0];
              for (c_i3 = 0; c_i3 < nrows; c_i3++) {
                a_data[c_i3 + a->size[0] * b_k] =
                    ktree_data[(int)(b_i3 - 1.0) - 1].f1->data[c_i3] *
        ...
        if (b_i2 + 1U <= (unsigned int)rx) {
          ...
          for (j = 0; j < i2; j++) {
            ...
            if (b_c != 0.0) {
              ...
              if (c_i3 - b_k == ktree_data[(int)k - 1].f1->size[0]) {
                nrows = (nrows - ibcol) + 1;
                c_i3 = indvec->size[0] * indvec->size[1];
                indvec->size[0] = 1;
                indvec->size[1] = nrows;
                emxEnsureCapacity_real_T(indvec, c_i3);
                indvec_data = indvec->data;
                for (c_i3 = 0; c_i3 < nrows; c_i3++) {
                  indvec_data[c_i3] = kweig_data[b_k + c_i3] +
                                      ktree_data[(int)k - 1].f1->data[c_i3];
                }
                ...
                n_binary_expand_op(kweig, ibcol, b_k, c_i3 - 1, ktree, k, nrows,
                                   ibcol - 1);
                kweig_data = kweig->data;
              }
            }
          ...
          if (b_k - i2 == ktree_data[(int)k - 1].f1->size[0]) {
            nrows = (ibcol - c_i3) + 1;
            ...
            for (b_k = 0; b_k < nrows; b_k++) {
              indvec_data[b_k] =
                  kweig_data[i2 + b_k] + ktree_data[(int)k - 1].f1->data[b_k];
            }
            loop_ub = indvec->size[1];
            for (i2 = 0; i2 < loop_ub; i2++) {
              kweig_data[c_i3 + i2] = indvec_data[i2];
            }
          } else {
            n_binary_expand_op(kweig, c_i3, i2, b_k - 1, ktree, k, ibcol,
                               c_i3 - 1);
            kweig_data = kweig->data;
          }
        }
