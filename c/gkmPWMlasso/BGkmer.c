            ...
            emxArray_real_T *negweights)
{
  ...
  len = (double)(c->size[0] * c->size[1]) / k;
  ...
  for (i = 0; i < b_j1; i++) {
    ...
    m = c->size[1];
    b_i = b_c->size[0] * b_c->size[1];
    b_c->size[0] = 1;
    b_c->size[1] = c->size[1];
    emxEnsureCapacity_real_T(b_c, b_i);
    b_c_data = b_c->data;
    for (b_i = 0; b_i < m; b_i++) {
      b_c_data[b_i] = c_data[i + c->size[0] * b_i];
    }
    diff(b_c, dc);
    dc_data = dc->data;
    /* 'BGkmer:29' startvec = GCmat*p{c(i,1)}; */
    for (b_i = 0; b_i < 16; b_i++) {
      b_p[b_i] = p_data[(int)c_data[i] - 1].f1[b_i];
    }
    for (b_i = 0; b_i < 4; b_i++) {
      i1 = b_i << 2;
      startvec[b_i] = ((GCmat_idx_0_tmp * b_p[i1] + GCmat_idx_1 * b_p[i1 + 1]) +
                       GCmat_idx_2 * b_p[i1 + 2]) +
                      GCmat_idx_0_tmp * b_p[i1 + 3];
    }
    /* 'BGkmer:30' if RC */
    if (RC != 0.0) {
      ...
      b_c_data = b_c->data;
      for (b_i = 0; b_i < m; b_i++) {
        b_c_data[b_i] = c2_data[i + c2->size[0] * b_i];
      }
      diff(b_c, dc2);
      dc2_data = dc2->data;
  ...
  if ((rcnum > 0.0) && (RC != 0.0)) {
    ...
    len = c_tmp * (len - rcnum) + 1.0;
    if (len > negweights->size[0]) {
      b_j1 = 1;
      b_i = -1;
      i1 = 0;
    } else {
    ...
    b_c_data = b_c->data;
    for (i1 = 0; i1 < m; i1++) {
      b_c_data[i1] = negweights_data[(b_j1 + i1) - 1] / 1.4142135623730951;
    }
    m = b_c->size[1];
    for (b_j1 = 0; b_j1 < m; b_j1++) {
