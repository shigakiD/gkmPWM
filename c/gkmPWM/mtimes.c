...
void c_mtimes(const emxArray_real_T *A, const emxArray_real_T *B, double C[4])
{
  ...
  if ((A->size[0] == 0) || (B->size[0] == 0)) {
    ...
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, (blasint)4, (blasint)1,
...
void d_mtimes(const emxArray_real_T *A, const double B[4], emxArray_real_T *C)
{
  ...
  if (A->size[0] == 0) {
    ...
    emxEnsureCapacity_real_T(C, i);
    C_data = C->data;
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (blasint)A->size[0],
                (blasint)1, (blasint)4, 1.0, &A_data[0], (blasint)A->size[0],
                &B[0], (blasint)4, 0.0, &C_data[0], (blasint)A->size[0]);
  }
}
