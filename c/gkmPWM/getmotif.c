              ...
              emxArray_cell_wrap_4 *mat, emxArray_cell_wrap_0 *names)
{
  ...
  if (filename->size[1] == 3) {
    ...
    do {
      exitg1 = 0;
      if (text_len < 3) {
        if (filename_data[text_len] != cv[text_len]) {
          exitg1 = 1;
        } else {
          text_len++;
        }
  ...
  if (fid < 0) {
    ...
    for (j = 0; j < i; j++) {
      ...
      for (i2 = 0; i2 < match_idx; i2++) {
        names_data[j].f1->data[i2] = line_clean_data[i1 + i2];
      }
      /*  a = strsplit(line, ' '); */
      /*  names{i} = a{end}; */
      ...
      while (line->size[1] != 0) {
        ...
        for (idx = 0; idx <= nz; idx++) {
          ...
          text_len = line->size[0] * line->size[1];
          line->size[0] = 1;
          match_idx = i2 - i1;
          line->size[1] = match_idx;
          emxEnsureCapacity_char_T(line, text_len);
          line_data = line->data;
          for (i2 = 0; i2 < match_idx; i2++) {
