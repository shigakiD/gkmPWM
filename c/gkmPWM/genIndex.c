...
static void fb_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *c,
                                int i, const emxArray_real_T *vec);

static void gb_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *C,
...
static void hb_binary_expand_op(emxArray_real_T *x, const emxArray_real_T *c,
  ...
  for (b_i = 0; b_i <= loop_ub - 2; b_i++) {
    ...
    if (d_data[b_i] != 0.0) {
      ...
      if (c->size[1] == vec->size[1]) {
        ...
        fb_binary_expand_op(x, c, b_i, vec);
        x_data = x->data;
      }
      nx = x->size[1];
      i = y->size[0] * y->size[1];
      ...
      if (sum(y) == 0.0) {
        ...
        for (j = 0; j <= i - 2; j++) {
          ...
          if (c->size[1] == vec->size[1]) {
            ...
            eb_binary_expand_op(x, c, b_j, vec);
            x_data = x->data;
          }
          nx = x->size[1];
  ...
  *rcnum = a - 1.0;
  ...
  for (b_i = 0; b_i <= i - 2; b_i++) {
    ...
    for (j = 0; j <= i1 - 2; j++) {
      ...
      if (C->size[1] == x->size[1]) {
        ...
        gb_binary_expand_op(x, C, f, d, b_i);
        x_data = x->data;
      }
      nx = x->size[1];
  ...
  for (b_i = 0; b_i < loop_ub_tmp; b_i++) {
    ...
    if (c->size[1] == C->size[1]) {
      ...
      hb_binary_expand_op(x, c, b_ind, f, b_i, C);
      x_data = x->data;
    }
    nx = x->size[1];
    i = y->size[0] * y->size[1];
