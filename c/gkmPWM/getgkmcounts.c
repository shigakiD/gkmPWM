...
static void c_binary_expand_op(emxArray_real_T *gkmc, double a,
                               const emxArray_real_T *temp);

static void d_binary_expand_op(emxArray_real_T *y, const emxArray_real_T *r3,
                               const emxArray_real_T *M);

static void e_binary_expand_op(emxArray_real_T *temp, int i1, int i2, int i3);
                        ...
                        emxArray_real_T *mat)
{
  ...
  /* 'getgkmcounts:269' lcnum = numel(c)/k; */
  lcnum = (double)(c->size[0] * c->size[1]) / k;
  /* 'getgkmcounts:270' seqvec = zeros(4^l, l); */
  c_tmp = pow(4.0, l);
  ...
  /* 'getgkmcounts:272' for i = 1:l */
  emxInit_real_T(&x, 1);
  for (b_i = 0; b_i < i; b_i++) {
    /* 'getgkmcounts:273' seqvec(:,i) = mod(floor(vec/4^(i-1)), 4); */
    xtmp = pow(4.0, ((double)b_i + 1.0) - 1.0);
    nx = vec->size[0];
    j2 = x->size[0];
    x->size[0] = vec->size[0];
    ...
    for (j2 = 0; j2 < nx; j2++) {
      xtmp = y_data[j2];
      seqvec_data[j2 + seqvec->size[0] * b_i] = b_mod(xtmp, 4.0);
    }
  }
  emxInit_real_T(&seqvec2, 2);
  /* 'getgkmcounts:275' seqvec2 = 3-fliplr(seqvec); */
  ...
  /* 'getgkmcounts:276' mat = zeros(4^l, 2*lcnum); */
  ...
  emxFree_real_T(&y);
  /* 'getgkmcounts:278' for i = 1:lcnum */
  j2 = (int)lcnum;
  emxInit_real_T(&a, 2);
  for (b_i = 0; b_i < j2; b_i++) {
    /* 'getgkmcounts:279' mat(:,i) = seqvec(:,c(i,:))*pow+4^k*(i-1)+1; */
    nx = seqvec->size[0];
    nd2 = c->size[1];
    i = a->size[0] * a->size[1];
    ...
    c_tmp = pow(4.0, k);
    xtmp = c_tmp * (((double)b_i + 1.0) - 1.0);
    nx = x->size[0];
    for (i = 0; i < nx; i++) {
      mat_data[i + mat->size[0] * b_i] = (y_data[i] + xtmp) + 1.0;
    }
    /* 'getgkmcounts:280' mat(:,i+lcnum) =
     * seqvec2(:,c(i,:))*pow+4^k*(lcnum+i-1)+1; */
    d = ((double)b_i + 1.0) + lcnum;
    nx = seqvec2->size[0];
    nd2 = c->size[1];
    i = a->size[0] * a->size[1];
    ...
    nx = x->size[0];
    for (i = 0; i < nx; i++) {
      mat_data[i + mat->size[0] * ((int)d - 1)] = (y_data[i] + xtmp) + 1.0;
    }
  }
  emxFree_real_T(&x);
  emxFree_real_T(&a);
                             ...
                             emxArray_real_T *mat)
{
  ...
  c_data = c->data;
  emxInit_real_T(&seqvec, 2);
  /* 'getgkmcounts:284' lcnum = numel(c)/k; */
  lcnum = (double)(c->size[0] * c->size[1]) / k;
  /* 'getgkmcounts:285' seqvec = zeros(4^l, l); */
  c_tmp = pow(4.0, l);
  ...
  /* 'getgkmcounts:287' for i = 1:l */
  emxInit_real_T(&x, 1);
  for (b_i = 0; b_i < i1; b_i++) {
    /* 'getgkmcounts:288' seqvec(:,i) = mod(floor(vec/4^(i-1)), 4); */
  ...
  emxFree_real_T(&x);
  /* 'getgkmcounts:290' mat = zeros(4^l, lcnum); */
  i = mat->size[0] * mat->size[1];
  mat->size[0] = (int)c_tmp;
  i1 = (int)lcnum;
  mat->size[1] = (int)lcnum;
  emxEnsureCapacity_real_T(mat, i);
  mat_data = mat->data;
  nx = (int)c_tmp * (int)lcnum;
  for (i = 0; i < nx; i++) {
    mat_data[i] = 0.0;
  }
  /* 'getgkmcounts:291' pow = 4.^(0:k-1)'; */
  if (k - 1.0 < 0.0) {
    y->size[1] = 0;
  } else {
    i = y->size[0] * y->size[1];
  ...
  /* 'getgkmcounts:292' for i = 1:lcnum */
  emxInit_real_T(&a, 2);
  for (b_i = 0; b_i < i1; b_i++) {
    /* 'getgkmcounts:293' mat(:,i) = seqvec(:,c(i,:))*pow+4^k*(i-1)+1; */
    nx = seqvec->size[0];
    b_k = c->size[1];
    i = a->size[0] * a->size[1];
    a->size[0] = seqvec->size[0];
    ...
    for (i = 0; i < b_k; i++) {
      for (i2 = 0; i2 < nx; i2++) {
        a_data[i2 + a->size[0] * i] =
            seqvec_data[i2 + seqvec->size[0] *
                                 ((int)c_data[b_i + c->size[0] * i] - 1)];
      }
    }
                  ...
                  double mat[16], double mat2[16])
{
  ...
  cell_wrap_0 validatedHoleFilling[3];
  cell_wrap_0 r;
  cell_wrap_0 r1;
  ...
  emxInitMatrix_cell_wrap_01(validatedHoleFilling);
  ...
  if (vlen != -1) {
    ...
    if (vlen != -1) {
      ...
      for (cur_idx = 0; cur_idx < vlen; cur_idx++) {
        /* 'getgkmcounts:41' sequences{cur_idx} = cellstr(""); */
        emxCopyStruct_cell_wrap_0(&sequences_data[cur_idx].f1[0], &r);
      }
      ...
      for (cur_idx = 0; cur_idx < vlen; cur_idx++) {
         ...
         */
        fgetl(fileid, r1.f1);
        emxCopyStruct_cell_wrap_0(&sequences_data[cur_idx].f1[0], &r1);
      }
  ...
  if (guard1) {
    ...
    if (vlen != -1) {
      ...
      while ((!exitg2) && (cur_idx <= (int)idx - 1)) {
        ...
        for (i = 0; i < 2; i++) {
          x_size[i] = validatedHoleFilling[2].f1->size[i];
        }
        y = (x_size[1] != 0);
        if (y) {
          y = (0 > x_size[1] - 1);
        ...
        if (y) {
          ...
          strip(validatedHoleFilling[1].f1, r.f1);
          emxCopyStruct_cell_wrap_0(&sequences_data[cur_idx].f1[0], &r);
          cur_idx++;
        }
      }
  ...
  /* 'getgkmcounts:125' lcnum = numel(comb)/k; */
  lcnum = (double)(comb->size[0] * comb->size[1]) / k;
  /* 'getgkmcounts:126' n = length(alpha); */
  ...
  *GCneg = 0.0;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if (ver == 0) {
      ...
      while ((!exitg2) && ('>' != validatedHoleFilling[1].f1->data[0])) {
        ...
        vlen = validatedHoleFilling[0].f1->size[0] *
               validatedHoleFilling[0].f1->size[1];
        validatedHoleFilling[0].f1->size[0] = 1;
        validatedHoleFilling[0].f1->size[1] +=
            validatedHoleFilling[1].f1->size[1];
        emxEnsureCapacity_char_T(validatedHoleFilling[0].f1, vlen);
        loop_ub = validatedHoleFilling[1].f1->size[1];
        for (vlen = 0; vlen < loop_ub; vlen++) {
          validatedHoleFilling[0].f1->data[vlen + i2] =
              validatedHoleFilling[1].f1->data[vlen];
        }
        ...
        if (a > sequences->size[0]) {
          ...
          vlen = (int)a - 1;
          validatedHoleFilling[1].f1->size[1] =
              sequences_data[vlen].f1[0].f1->size[1];
          emxEnsureCapacity_char_T(validatedHoleFilling[1].f1, i2);
          loop_ub = sequences_data[vlen].f1[0].f1->size[1];
          for (i2 = 0; i2 < loop_ub; i2++) {
            validatedHoleFilling[1].f1->data[i2] =
                sequences_data[vlen].f1[0].f1->data[i2];
          }
        }
      }
    } else {
    ...
    for (j = 0; j < i2; j++) {
      ...
      r6->size[0] = 1;
      r6->size[1] = x->size[1];
      emxEnsureCapacity_int32_T(r6, vlen);
      r3 = r6->data;
      for (vlen = 0; vlen < loop_ub; vlen++) {
        r3[vlen] = (int)x_data[((int)(curr_pos + 1.0) + x->size[0] * vlen) - 1];
      }
      ...
      for (vlen = 0; vlen < loop_ub; vlen++) {
        M_data[vlen] =
            temp_data[(int)x_data[((int)(curr_pos + 1.0) + x->size[0] * vlen) -
                                  1] -
                      1] +
            1.0;
      }
      loop_ub = c_y->size[1];
      for (vlen = 0; vlen < loop_ub; vlen++) {
        temp_data[r3[vlen] - 1] = M_data[vlen];
      }
    }
    ...
    if (RC != 0.0) {
      ...
      if (loop_ub == vlen - i2) {
        for (vlen = 0; vlen < loop_ub; vlen++) {
          temp_data[vlen] += temp_data[i2 + vlen];
        }
        i2 = temp->size[0];
        temp->size[0] = loop_ub;
        emxEnsureCapacity_real_T(temp, i2);
        ...
        e_binary_expand_op(temp, loop_ub - 1, i2, vlen - 1);
        temp_data = temp->data;
      }
    ...
    if ((rcnum > 0.0) && (RC != 0.0)) {
      ...
      if (d > curr_pos) {
        ...
        i2 = (int)d;
        vlen = (int)d - 2;
        cur_idx = (int)curr_pos;
      }
      x_size[1] = (cur_idx - vlen) - 1;
      cur_idx = c_y->size[0] * c_y->size[1];
      c_y->size[0] = 1;
      c_y->size[1] = x_size[1];
      emxEnsureCapacity_real_T(c_y, cur_idx);
      M_data = c_y->data;
      loop_ub = x_size[1];
      for (cur_idx = 0; cur_idx < loop_ub; cur_idx++) {
        M_data[cur_idx] = temp_data[(i2 + cur_idx) - 1] / 1.4142135623730951;
      }
      loop_ub = c_y->size[1];
      for (i2 = 0; i2 < loop_ub; i2++) {
        temp_data[(vlen + i2) + 1] = M_data[i2];
      }
    }
    ...
    if (LK == 1) {
      ...
      if (gkmc->size[0] == temp->size[0]) {
        ...
        c_binary_expand_op(gkmc, curr_pos, temp);
        gkmc_data = gkmc->data;
      }
      ...
      for (b_k = 0; b_k < i2; b_k++) {
        ...
        for (j = 0; j < vlen; j++) {
          ...
          cur_idx = b_k - j;
          if ((cur_idx + 1 <= curr_pos) && (cur_idx + 1 > 0)) {
            /* 'getgkmcounts:202' C{ss(ii)+1,1}(ii-j,j+1) = 1; */
            C[(int)(ss_data[b_k] + 1.0) - 1]
                .f1->data[cur_idx +
                          C[(int)(ss_data[b_k] + 1.0) - 1].f1->size[0] * j] =
                1.0;
          }
        }
      }
      ...
      if (RC != 0.0) {
        ...
        for (b_k = 0; b_k < 4; b_k++) {
          ...
          M2->size[0] = C[3 - b_k].f1->size[0];
          M2->size[1] = C[3 - b_k].f1->size[1];
          emxEnsureCapacity_real_T(M2, i2);
          M2_data = M2->data;
          loop_ub = C[3 - b_k].f1->size[0] * C[3 - b_k].f1->size[1] - 1;
          for (i2 = 0; i2 <= loop_ub; i2++) {
            M2_data[i2] = C[3 - b_k].f1->data[i2];
          }
          rot90(M2, C[b_k + 4].f1);
        }
      ...
      for (b_k = 0; b_k < 4; b_k++) {
        ...
        mtimes(C[b_k].f1, C[b_k].f1, b_Ker);
        ...
        if (RC != 0.0) {
          ...
          mtimes(C[b_k].f1, C[b_k + 4].f1, b_Ker);
          b_Ker_data = b_Ker->data;
      ...
      if (RC != 0.0) {
        ...
        if ((b_Ker->size[0] == M->size[0]) && (b_Ker->size[1] == M->size[1])) {
          ...
          d_binary_expand_op(c_y, b_Ker, M);
        }
    ...
    if (alpha_data[b_i] > 0.0) {
      ...
      *GCpos += alpha_data[b_i] * ((double)cur_idx + (double)loop_ub);
      ...
      *GCneg += alpha_data[b_i] * ((double)cur_idx + (double)loop_ub);
  ...
  emxFreeMatrix_cell_wrap_01(validatedHoleFilling);
  emxFree_real_T(&c_y);
  emxFree_int32_T(&r6);
  emxFree_real_T(&r4);
  emxFree_cell_wrap_10(&sequences);
  emxFree_real_T(&M);
  emxFreeMatrix_cell_wrap_4(C);
  emxFree_real_T(&ss);
  emxFree_real_T(&M2);
  emxFree_real_T(&Ker);
  emxFree_real_T(&x);
...
void letterconvert(const emxArray_char_T *s, emxArray_real_T *en)
{
  ...
  char c;
  s_data = s->data;
  /* 'getgkmcounts:254' l = length(s); */
  /* 'getgkmcounts:255' en = zeros(1,l); */
  i = en->size[0] * en->size[1];
  en->size[0] = 1;
  en->size[1] = s->size[1];
  emxEnsureCapacity_real_T(en, i);
  en_data = en->data;
  /* 'getgkmcounts:256' for i = 1:l */
  i = s->size[1];
  for (b_i = 0; b_i < i; b_i++) {
    /* 'getgkmcounts:257' if strcmp(s(i),'A') || strcmp(s(i), 'a') */
    c = s_data[b_i];
    if ((!(c != 'A')) || (!(c != 'a'))) {
      /* 'getgkmcounts:258' en(i) = 0; */
      en_data[b_i] = 0.0;
    } else if ((!(c != 'C')) || (!(c != 'c'))) {
      /* 'getgkmcounts:259' elseif strcmp(s(i),'C') || strcmp(s(i),'c') */
      /* 'getgkmcounts:260' en(i) = 1; */
      en_data[b_i] = 1.0;
    } else if ((!(c != 'G')) || (!(c != 'g'))) {
      /* 'getgkmcounts:261' elseif strcmp(s(i),'G') || strcmp(s(i),'g') */
      /* 'getgkmcounts:262' en(i) = 2; */
      en_data[b_i] = 2.0;
    } else {
      /* 'getgkmcounts:263' else */
      /* 'getgkmcounts:264' en(i) = 3; */
      en_data[b_i] = 3.0;
    }
  }
}
